...
export interface StatsSummary {
  ...
  months: { month: number; count: number }[]
  /**
   * FEAT-21 — იგივე რიცხვი მოდულებად (წლიური მიზნის პროგრესისთვის).
   * ⚠️ **ნულებით შევსებულია**: გამოტოვებული გასაღები „მიზანი არ არსებობს"-ად
   * წაიკითხებოდა და არა „ჯერ არაფერი დაგისრულებია".
   */
  done_by_module: Record<string, number>
  /**
   * რომელ მოდულს **შეიძლება** ჰქონდეს მიზანი.
   * ⚠️ მხოლოდ ისინი, ვისაც „როდის დავასრულე" თარიღი აქვს — წიგნს,
   * თამაშს, სამაგიდოსა და ჩანაწერს ის არ აქვს, ე.ი. პროგრესი
