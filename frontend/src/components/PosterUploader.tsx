...
export function PosterUploader({
  ...
  return (
    <div
      {...getRootProps()}
      className={cn(
        'flex aspect-[2/3] w-40 cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border p-3 text-center text-xs text-muted-foreground transition-colors hover:border-primary hover:bg-muted',
        isDragActive && 'border-primary bg-muted text-foreground',
      )}
    >
      <input {...getInputProps()} />
      <UploadCloud className="size-7 opacity-60" />
      <span>{t('form.posterHint')}</span>
    </div>
  )
}
