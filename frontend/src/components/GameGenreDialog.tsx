...
/* ============================================================
   თამაშის ჟანრის დამატება/რედაქტირება.

   იგივე მოდალი ორ ადგილას მუშაობს (როგორც წიგნებსა და სიმღერებზე):
   `/game-genres` და ფორმის „+ ახალი ჟანრი".
   ============================================================ */

export function GameGenreDialog({
  genre,
  onClose,
  onSaved,
}: {
  ...
  onClose: () => void
  onSaved?: (saved: GameGenre) => void
}) {
  const { t } = useTranslation()
  const qc = useQueryClient()
  const { toast } = useToast()

  const [form, setForm] = useState<GameGenreInput>({
    name_ka: genre?.name_ka ?? '',
    name_en: genre?.name_en ?? '',
    icon: genre?.icon ?? 'Gamepad2',
  })
  const [errors, setErrors] = useState<Record<string, string>>({})

  const save = useMutation({
    mutationFn: (input: GameGenreInput) =>
      genre ? updateGameGenre(genre.id, input) : createGameGenre(input),
    onSuccess: (saved) => {
      qc.invalidateQueries({ queryKey: ['game-genres'] })
      qc.invalidateQueries({ queryKey: ['games'] })
      toast({ title: t('gameGenres.saved'), variant: 'success' })
      onSaved?.(saved)
      onClose()
    },
    onError: (e) => {
  ...
  return (
    <ModalShell title={t(genre ? 'gameGenres.edit' : 'gameGenres.add')} onClose={onClose} wide>
      <form onSubmit={submit} className="mt-4 space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <Label htmlFor="gg-ka">{t('genres.name_ka')}</Label>
            <Input
              id="gg-ka"
              autoFocus
              value={form.name_ka}
          ...
          <div>
            <Label htmlFor="gg-en">{t('genres.name_en')}</Label>
            <Input
              id="gg-en"
              value={form.name_en}
