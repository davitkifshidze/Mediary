...
| GAP-07 | `PUBLIC_PROFILES=false` `/matches`-ს არ თიშავს | Low | gap | S | ✅ შესრულებულია |
...
| FEAT-01 | ტესტი: backend-ის ყველა მანქანური კოდი ⊆ `CODES` ⊆ ორივე ლოკალი | Backlog | feature | S | ✅ შესრულებულია |
| FEAT-02 | `mediary:seed-demo` — ანონიმური საჩვენებელი მონაცემები კომიტებული dump-ის ნაცვლად | Backlog | feature | M | ✅ შესრულებულია |
| FEAT-03 | პარტიის თითო ერთეულის შედეგი (`ok`/`skipped`/`failed` + მიზეზი) და მისი ჩვენება SPA-ში | Backlog | feature | M | ✅ შესრულებულია |
| FEAT-04 | ალბომის პაროლის წარუმატებელი ცდების DB-მრიცხველი და დროებითი დაბლოკვა | Backlog | feature | S | ✅ შესრულებულია |
| FEAT-05 | `mediary:doctor` — ბინარების, scheduler-ის და storage-მრიცხველის დრიფტის ერთი შემოწმება | Backlog | feature | M | ✅ შესრულებულია |
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `NoteModuleTest::test_a_due_reminder_is_claimed_by_only_one_runner`, 31/31 მწვანე.
  - ⚠️ **რეჟიმი განზრახ `daily`-ია და არა `once`.** არსებული `test_due_reminder_fires_once_and_deactivates` სწორედ ამიტომ ვერ იჭერდა claim-ს: `once`-ზე პირველი გასროლის შემდეგ `is_active` false ხდება, ე.ი. მეორე ცდას `where('is_active', true)` აჩერებს და `where('next_at', …)` საერთოდ არ მონაწილეობს. განმეორებადზე შეხსენება აქტიური რჩება — ე.ი. compare-and-swap ერთადერთი მცველია
  - ⚠️ **ორივე გამომძახებელი ერთსა და იმავე snapshot-ს კითხულობს.** `run()`-ის ორჯერ გამოძახება რბოლას **ვერ აღადგენს**: ის ყოველ ჯერზე ხელახლა კითხულობს, ე.ი. მეორე უკვე გადაწეულ რიგს დაინახავდა და ტესტი გადაწევას შეამოწმებდა და არა claim-ს. ამიტომ `fire()` რეფლექსიით ორჯერ იძახება ორ ცალკე, ერთნაირად წაკითხულ მოდელზე — ზუსტად ის, რასაც cron-ისა და ღია ტაბის ერთდროული `SELECT` ქმნის
  - ✅ მტკიცებები: პირველი `true`, მეორე `false`, **ზუსტად ერთი** `NoteNotification`, და შეხსენება **ისევ აქტიურია** `next_at = 2026-09-05T09:00:00Z`-ით — ეს უკანასკნელი ადასტურებს, რომ მეორე ცდა ნამდვილად `next_at`-მა შეაჩერა
  - ✅ **მუტაცია:** `where('next_at', $reminder->next_at)` მოეხსნა → **მხოლოდ ეს ერთი ტესტი** წითლდება (30 დანარჩენი მწვანეა), რაც თვითონვე ადასტურებს, რომ ძველი ტესტი ამ ხაზს არ ფარავდა
- **ტიპი:** debt
...
- **Acceptance criteria:**
  - [x] ტესტი წერია და მწვანეა; claim-ის შეცვლა (`where('next_at')`-ის ამოღება) მას აწითლებს
- **Estimate:** S
- **დამოკიდებულება:** none

### [DEBT-12] `NoteReminders.test.ts` სრულ `npm test`-ში 5-წამიან ტაიმაუტზე ცვივა (ცალკე გადის) — CI-ს flaky-ს ხდის
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — სრული `npm test` **სამ ზედიზედ რანში 161/161** (27 ფაილი, 8.4–9.0 წმ).
  - ✅ **ჯერ გამეორდა:** `npm test` 7 წითელს იძლეოდა — პირველი `Test timed out in 5000ms`, დანარჩენი ექვსი ჯაჭვურად (`expected '' to contain …`, `Cannot read properties of undefined`).
  - ✅ **მიზეზი გაზომილია და არა ნავარაუდევი:** ფაილის ცალკე გაშვებაზე **პირველი** ტესტი **3578 ms**-ია, დანარჩენი ექვსი 4–72 ms. ე.ი. ხარჯი ერთჯერადი იმპორტი/პირველი რენდერია (`TimePicker` → react-aria + `@internationalized/date`, `DatePicker` → react-day-picker), და არა თვითონ ტესტი. სრულ რანში 19 jsdom გარემო პარალელურად იქმნება (Vitest-ის საკუთარი ანგარიშით დროის 66%) — 3.6 წმ 5-ს სცდება.
  - ✅ **გასწორება ხარჯს ბიუჯეტიდან იღებს და არა რიცხვს ზრდის:** `beforeAll`-ში ორივე კომპონენტის მოდულის `await import()` (საკუთარი, ცხადი ვადით). შედეგი გაზომილია: პირველი ტესტი **3578 ms → 245 ms**.
  - ✅ `vi.setConfig({ testTimeout: 20_000 })` სარეზერვოდ, მიზეზის კომენტარით — და **ის მართლაც სარეზერვოა, არა დეკორაცია**: გათბობის დროებით მოხსნაზე სრული რანი მაინც 161/161 დარჩა, ე.ი. ორივე ფენა დამოუკიდებლად მუშაობს.
  - ✅ **ჯაჭვური ექვსი წითელიც აიხსნა და ჩაიწერა:** ტაიმაუტი `act()`-ის შიგნით ხვდება, React-ის act-რიგი ჩარჩენილი რჩება და მომდევნო `render()`-ები ცარიელ კონტეინერს აბრუნებენ — ამიტომ ერთი ნელი მანქანა შვიდ წითელს ხატავს ლოგიკის ერთი ცვლილების გარეშე.
  - ⚠️ **`pool: 'vmThreads'` გაიზომა და განზრახ არ დაემატა.** Vitest-ი მას თვითონ გვთავაზობს, და ის მართლა ამცირებს გარემოს წილს (66% → 37%), მაგრამ **საერთო დრო არ იცვლება** (9.02 წმ vs 8.4–9.0 წმ) — მხოლოდ გადაანაწილებს. `vmThreads`-ს კი ცნობილი ფასი აქვს (`node:vm`-ის შეზღუდვები, მეხსიერების გაჟონვა, ფაილებს შორის გაზიარებული გლობალები — ამ ფაილებში კი მოდულის დონის მოკები და `IS_REACT_ACT_ENVIRONMENT` ზის). ნულოვანი მოგებისთვის რეალურ რისკს არ ვიხდით; თუ ოდესმე ტესტების რაოდენობა გაიზრდება, გაზომვა თავიდან ღირს.
  - ℹ️ ტესტების რაოდენობა ტასკის დაწერის შემდეგ 125 → **161** გაიზარდა; სემანტიკა არც ერთზე არ შეცვლილა — არც ერთი მტკიცება არ შეხებია.
- **ტიპი:** debt
...
- **Acceptance criteria:**
  - [x] სრული `npm test` 3 ზედიზედ რანში მწვანეა (161/161 — რაოდენობა ტასკის დაწერის შემდეგ გაიზარდა)
  - [x] ტესტის სემანტიკა უცვლელია (ტაიმაუტის ზრდა მიზეზის კომენტარით)
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `CustomFieldTest` 30/30.
  - ✅ `created_at` მხოლოდ insert-ზე იწერება: `updateOrInsert()`-ის მეორე არგუმენტი **closure-ია** და `$exists`-ს იღებს (Laravel 13-ის ხელმოწერა `array|callable`)
  - ⚠️ **დამატებითი query არ ჩნდება** — `$exists` სწორედ იმ `exists()`-იდან მოსდის, რომელსაც `updateOrInsert()` ისედაც აკეთებს; ხელით „ჯერ შევამოწმოთ, მერე ჩავწეროთ" ორ გამოკითხვას დაბადებდა და რბოლასაც ხსნიდა
  - ⚠️ **ეს არა მხოლოდ სისუფთავეა:** `StorageMeter::files()` ამ ცხრილებიდან `created_at`-ს **„ატვირთვის თარიღად"** კითხულობს, ე.ი. საცავის ბიბლიოთეკაში custom-field ფაილის თარიღი ჩანაწერის ყოველ **უკავშირო** შენახვაზე წინ მიცოცავდა და სორტირებაც მას მიჰყვებოდა
  - ⚠️ ტესტში **დრო ორ სხვადასხვა მომენტზე იყინება** (`setTestNow`): ერთ მომენტში ორივე ჩაწერა ერთსა და იმავე `created_at`-ს დაწერდა და ტესტი გატეხილ კოდზეც გაივლიდა. `updated_at` კი ცხადად **უნდა** დაიძრას — ორივე მტკიცებაა
  - ℹ️ პროექტში მეორე `updateOrInsert` `BackupRunner:196`-ია და ის **სწორია**: აღდგენის შემდეგ ის რიგის **სნეპშოტს** აბრუნებს, ე.ი. ორიგინალი `created_at` სწორედ უნდა ჩაიწეროს
  - ✅ **მუტაცია:** ფიქსირებული `'created_at' => now()` დაბრუნდა → ტესტი წითლდება („2026-09-01 14:00:00" ≠ „2026-09-10 14:00:00")
- **ტიპი:** bug
...
- **Acceptance criteria:**
  - [x] ტესტი: მნიშვნელობის მეორე ჩაწერა `created_at`-ს არ ცვლის
- **Estimate:** S
- **დამოკიდებულება:** none

### [BUG-13] `deleteResolved()`-ის custom-field ბრანჩი: დისკი + მრიცხველი + row ტრანზაქციის გარეშე
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `StorageManagementTest` 24/24, `CustomFieldTest` 30/30.
  - ✅ **რიგი და მრიცხველი ერთ `DB::transaction()`-ში, ფაილი — commit-ის შემდეგ** (ზუსტად ის რიგი, რაც `AlbumVault`-ს აქვს BUG-03/BUG-04-ის შემდეგ)
  - ⚠️ **მიმდევრობა ისეთივე მნიშვნელოვანია, როგორც თვითონ ტრანზაქცია.** ძველად ფაილი **პირველი** იშლებოდა, ე.ი. შუაში ჩავარდნაზე ფაილი გამქრალია, კვოტა ჩამოკლებული, რიგი კი კვლავ `value_path`/`value_size`-ს აცხადებს — და მომდევნო `recalculate()` არარსებული ფაილის ბაიტებს **ხელახლა ამატებს**. საპირისპირო მიმდევრობა უვნებელია: commit-ის შემდეგ დისკის ჩავარდნა მხოლოდ **ობოლ ფაილს** ტოვებს, რომელსაც ადმინის ობოლების სკანერი იბრუნებს — ბაზა კი სწორია
  - ✅ ტესტი ჩავარდნას `addFor()`-ით ინჟექტირებს და ეს **ხელოვნური არაა**: ის ტრანზაქციის შიგნით და `DELETE`-ის **შემდეგ** დგას, ე.ი. ზუსტად იმ მდგომარეობას ქმნის, რომელიც უნდა დაბრუნდეს. სამივე ფაქტი ერთად მოწმდება — ფაილი ადგილზეა, რიგი ადგილზეა, მრიცხველი უცვლელია
  - ⚠️ ტესტი **`fail()`-ით იწყება გამონაკლისის არყოფნაზე**: `deleteOwnFile()` უცნობ ბილიკზე უბრალოდ `false`-ს აბრუნებს, ე.ი. ბილიკის შეცდომაზე სამივე მტკიცება უაზროდ გაივლიდა. ბოლოში კი **წარმატებული** წაშლაც მოწმდება — თორემ „ფაილი ისევ არის" იმის მტკიცებაც იქნებოდა, რომ ის საერთოდ არ იშლება
  - ✅ **მუტაცია:** ძველი სამნაბიჯიანი მიმდევრობა დაბრუნდა → ტესტი წითლდება („ფაილი commit-ამდე წაიშალა")
  - ℹ️ **მოდელური ბრანჩი შეგნებულად არ შეხებია.** აუდიტი მას `StoredFile`-ის ჰუკის გამო უსაფრთხოდ თვლის; სიმართლე ისაა, რომ იქაც `deleting` ფაილს `DELETE`-**მდე** შლის, ე.ი. ფანჯარა თეორიულად იგივეა — მაგრამ ეს ტრეიტის ცვლილებაა და **ყველა მოდულს ეხება**, ე.ი. ცალკე გადაწყვეტილებაა და არა ამ ტასკის ნაწილი
- **ტიპი:** bug
...
- **Acceptance criteria:**
  - [x] ტესტი: ტრანზაქციის ჩავარდნაზე ფაილი, რიგი და მრიცხველი — სამივე უცვლელია
- **Estimate:** S
- **დამოკიდებულება:** none

### [BUG-14] `errorMessage()` 422-ზე ვალიდაციის ტექსტს კოდზე წინ აყენებს
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `errors.test.ts` 14/14, სრული `npm test` 164/164, `tsc`/lint/build მწვანე.
  - ✅ `data.message` ჯერ `CODES`-ში მოწმდება; `first` (ვალიდაციის ტექსტი) მხოლოდ მაშინ იმარჯვებს, როცა კოდი არაა
  - ⚠️ **ჩვეულებრივი ვალიდაცია განზრახ არ შეცვლილა, და ეს გასწორების უსაფრთხოების მთელი არგუმენტია**: Laravel-ის `ValidationException` `message`-ში **პირველივე შეცდომის ტექსტს** წერს, ე.ი. ის `CODES`-ში ვერ მოხვდება და ჯაჭვი ისევ `first`-ს აბრუნებს. უამისოდ ეს ცვლილება ყველა ფორმას გაუფუჭებდა შეტყობინებას — ამიტომ ცალკე ტესტიც დაიწერა
  - ✅ სამი ტესტი: კოდი იმარჯვებს ვალიდაციის ტექსტზე · კოდის **პარამეტრებიც** ივსება, როცა `errors` ჩანთაც არის (თორემ ტექსტი „{{quota}}"-ით დაიხატებოდა) · ჩვეულებრივი ვალიდაცია ისევ ველის შეცდომას აბრუნებს
  - ✅ **მუტაცია:** `first ?? code` (ძველი რიგი) → ორივე პირველი ტესტი წითლდება
- **ტიპი:** bug
...
- **Acceptance criteria:**
  - [x] Vitest: `{message:'storage_quota_exceeded', errors:{file:['x']}}` თარგმნილ ტექსტს აბრუნებს
- **Estimate:** S
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `PurgeTest` 21/21.
  - ✅ `plan()`-ს დაემატა არჩევითი `?array $ids`; `run()` ჯერ **ერთხელ** ითვლის სკოუპს და მას გადასცემს
  - ⚠️ **ორმაგი ხარჯი აქ მეორეხარისხოვანია**: მთავარი ისაა, რომ ორ გამოძახებას შორის ჩაწერილი ჩანაწერი **ორ სხვადასხვა სეტს** დაბადებდა, ე.ი. „დათვლილი" და „წაშლილი" დაშორდებოდა — ზუსტად ის, რასაც კლასის მთავარი წესი კრძალავს
  - ⚠️ ტესტი query-ებს **`movies`-ზე** ითვლის და არა სულ: საერთო რიცხვს ყოველი ახალი მრიცხველი ან eager load გადასწევდა და ტესტი უკავშირო ცვლილებების მავთულსაკაბელი გახდებოდა. ფილტრი `recordIds()`-ის ხელწერაზეა (`select "id" from "movies"`), ჩანაწერების წამოღება (`select *`) სხვა query-ია
  - ✅ **მუტაცია:** ძველი ორმაგი გამოძახება დაბრუნდა → ტესტი წითლდება და ორივე query-ს სახელდებით ბეჭდავს
- **ტიპი:** performance
...
- **Acceptance criteria:**
  - [x] `run()`-ში `recordIds()` ერთხელ ეშვება (`PurgeTest` query-count)
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `photo-grid.test.ts` 2/2, სრული `npm test` 171/171.
  - ✅ `resolved` **ref-ია** და არა state; გადახატვის ბიძგი (`slideVersion`) მხოლოდ მაშინ, როცა **ლაითბოქსი გახსნილია** — დახურულზე რუკას არავინ კითხულობს
  - ⚠️ **რენდერისთვის რუკა საერთოდ არ იყო საჭირო**: უჯრა თავის მისამართს თვითონ იღებს (`usePrivateFileUrl`), რუკას კი მხოლოდ ორი მკითხველი ჰყავს — `slides` (გახსნილი ხედი) და `download()` (ივენთ-ჰენდლერი)
  - ⚠️ `noteResolved`-ის დამოკიდებულება **`lightboxOpen`-ია და არა ref**: ასე callback-ის იგივეობა სესიაზე ორჯერ იცვლება (გახსნა/დახურვა) და არა ყოველ მისამართზე, ხოლო რენდერში ref-ის ჩაწერა (რასაც React არ ურჩევს) საერთოდ არ გვჭირდება. `onResolved` ახლა `id`-საც იღებს, სწორედ იმიტომ, რომ სტაბილური იყოს
  - ⚠️ **ტესტმა ორი მცდარი გზა გაიარა და ორივე ჩაწერის ღირსია:** (ა) მრიცხველი ჯერ **მშობელზე** იდგა — `PhotoGrid`-ის შიდა `setState` მშობელს არ ეხება, ე.ი. მუტაციაზეც მწვანე იყო; (ბ) **უჯრების** მრიცხველი უკეთესია, მაგრამ მასში თითოეული უჯრის საკუთარი განახლებაც შედის, ე.ი. გატეხილი/გასწორებული მხოლოდ 4N/3N-ით სხვაობდა. საბოლოოდ ითვლება **ბადის საკუთარი** რენდერები — `NumberPick` მოკით, რომელიც ხელსაწყოთა ზოლშია
  - ⚠️ **თითო ტაიმერი ცალკე `act`-ში გადის**: ერთ `act`-ში React ყველა განახლებას აბატჩავს, ე.ი. გატეხილი კოდიც თითქმის ერთ რენდერს ხატავდა და ტესტი ვერაფერს გაარჩევდა
  - ✅ **მუტაცია:** `lightboxOpen` პირობა მოეხსნა → `grid renders = 5`, ზღვარი 2 — წითლდება დეტერმინისტულად
  - ✅ **მეორე ტესტი ლაითბოქსს ამოწმებს**: უამისოდ „ბადე აღარ გადაიხატება" მაშინაც მართალი იქნებოდა, თუ გახსნილი ხედი ცარიელი დარჩა — სწორედ ეს იყო ref-ზე გადასვლის ერთადერთი რეალური რისკი
  - ℹ️ **გაზომილი სხვაობა harness-ში მოკრძალებულია** (2 vs 5 რენდერი) და ტასკის „კვადრატულად იზრდება" აქ არ ჩანს: jsdom-ის ტაიმერები ერთმანეთს ძალიან ახლოს გადიან და React-ის ავტო-ბატჩინგი მათ ერთიანებს. ნამდვილ ბრაუზერში თითოეული blob ცალკე ქსელური პასუხია, ე.ი. ბატჩირება არ ხდება და სხვაობა უფრო დიდია
  - ✅ `eslint-disable` კომენტარი `slides`-ზე დარჩა (ref deps-ში ვერ იქნება), მაგრამ ახლა **მიზეზი წერია** და deps-ში ორივე რეალური ტრიგერია (`open`, `slideVersion`)
- **ტიპი:** performance
...
- **Acceptance criteria:**
  - [x] N tile-ის გახსნა `PhotoGrid`-ს ≤ 2-ჯერ ხატავს (render-მრიცხველი ტესტში)
- **Estimate:** S
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — შენი გადაწყვეტილება: **გადამრთველი მატჩინგსაც თიშავს**. `MatchTest` 18/18.
  - ✅ ახალი `PublicProfileService::enabled()` — გადამრთველს ახლა **ერთი მკითხველი** ჰყავს, და `resolve()` თვითონ მას იძახებს
  - ✅ `MatchService::ranking()` იწყება ამ შემოწმებით და ცარიელ კატალოგს აბრუნებს
  - ⚠️ **რატომ სწორედ `ranking()` იყო ხვრელი**: ის username-ით არავის ეძებს, ე.ი. `resolve()`-ს არ იძახებს — `show()`/`items()` კი იძახიან და ისედაც დაფარულები იყვნენ
  - ⚠️ **პასუხი ცარიელი კატალოგია და არა 404/403**: `/people` მოდულური გვერდი არაა და მისი გატეხვა არ გვინდა — გამორთულზე ის უბრალოდ ცარიელია, ზუსტად ისე, როგორც არც ერთი საჯარო პროფილი რომ არ არსებობდეს
  - ⚠️ შემოწმება **სერვისშია და არა კონტროლერში**: მეორე გამომძახებელი მას სხვაგვარად ვერ აუვლის გვერდს
  - ✅ ტესტი ჯერ **ჩართულზე** ამოწმებს, რომ დამთხვევა ნამდვილად არსებობს (თორემ „ცარიელია" ცარიელ ბაზაზეც მართალია), მერე თიშავს; წყვილურ endpoint-ებსაც ამოწმებს, რომ წესი მთელ მექანიზმზე ერთი იყოს
  - ✅ **მუტაცია:** მცველი მოეხსნა → ტესტი წითლდება და `bob`-ის სრულ ბარათს ბეჭდავს
- **ტიპი:** gap
...
- **Acceptance criteria:**
  - [x] გადაწყვეტილება მიღებულია (გადამრთველი მატჩინგსაც თიშავს) და ტესტითაც და docblock-ითაც დაფიქსირებულია
- **Estimate:** S
- **დამოკიდებულება:** none

### [GAP-08] გახსნილი ჩაკეტილი ალბომის ფოტო `Cache-Control`-ის გარეშე ბრუნდება
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `PublicGalleryTest` 14/14.
  - ✅ ჩაკეტილი ალბომის ფოტოს პასუხს ახლა `private, no-store, max-age=0` აქვს
  - ⚠️ **`private` და `no-store` ერთად, არა ერთ-ერთი**: პირველი შუამავალს (CDN/proxy) კრძალავს, მეორე — ბრაუზერის დისკსაც. ფოტო აქ მხოლოდ იმიტომ გამოდის, რომ პაროლი **ამ სესიაში** შეიყვანეს, ე.ი. კეშირებული ასლი ორივე მიმართულებით გაჟონავს — სხვასთანაც და პაროლის მოხსნის შემდეგაც
  - ⚠️ **მხოლოდ ჩაკეტილზე და არა მთელ მარშრუტზე**: დანარჩენი ფოტოები განზრახ საჯაროა და მათი კეშირება სასურველია — უამისოდ „უსაფრთხოება" მთელ საჯარო გალერეას ნელს ხდიდა
  - ✅ **ორი ტესტი და არა ერთი**: მეორე ცხადად ამბობს, რომ ჩვეულებრივ ფოტოს `no-store` **არ** აქვს — სწორედ ეს წყვილი ამტკიცებს, რომ ჰედერი ჩაკეტვის შედეგია და არა მარშრუტის მუდმივი თვისება
  - ✅ **მუტაცია:** `$locked = false` → ტესტი წითლდება (`'no-cache, private'` არ შეიცავს `no-store`-ს)
  - ℹ️ ეს **ძველ `/storage/...` მისამართს არ ეხება** (CLAUDE.md-ის „ვერ ვაკეთებთ" პუნქტი ძალაში რჩება): იქ ფაილს Laravel საერთოდ არ ემსახურება. ახალი როუტი კი იმავე კლასის ხვრელს თავიდან შემოიტანდა და სწორედ ისაა დახურული
- **ტიპი:** gap
...
- **Acceptance criteria:**
  - [x] `PublicGalleryTest`: ჩაკეტილი ალბომის ფოტოს პასუხს `no-store` აქვს (და ჩვეულებრივს — არა)
- **Estimate:** S
- **დამოკიდებულება:** none

### [GAP-09] ლექსიკონის წაშლისას `move_to: null`, გამოტოვება და self ერთსა და იმავეს ნიშნავს — გადაწყვეტილება სჭირდება
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — შენი გადაწყვეტილება: **(a) ცხადი გადამრთველი**. backend 774/774, frontend 171/171.
  - ✅ სამივე განზრახვას თავისი ცხადი შესატანი აქვს: `move_to` · **`clear_records`** · `delete_records`
  - ✅ `move_to` **სავალდებულოა**, თუ ორი ფლაგიდან არც ერთი არ მოვიდა → **422 `move_target_required`**; თავის თავზე გადატანა → **422 `move_target_is_self`**; `clear_records` + `delete_records` ერთად → 422
  - ⚠️ **`isset()`-ის შემთხვევითობა იყო მიზეზი**: `null`-ზე ის false-ია, ე.ი. **გასაღების არყოფნა**, ცხადი `null` და `<self>` სამივე „ცარიელი დარჩეს"-ად იკითხებოდა — ე.ი. ნახევრად აწყობილი ფორმა და ბაგიანი კლიენტი უხმოდ იღებდა იმ მდგომარეობას, რომელსაც სავალდებულო სტატუსის წესი კრძალავს
  - ⚠️ **„ცარიელი" არ აკრძალულა და არც უნდა აკრძალულიყო** — სამი არჩევანი შენი პასუხია 2026-09-13-დან; შეიცვალა მხოლოდ ის, რომ ის **ითქმება** და არა გამოტოვებით მოიპოვება
  - ⚠️ `rules()`-ს მესამე არგუმენტი დაემატა (`$selfId`) და თანმხლები `messages()` — **მანქანური კოდებით და არა ინგლისური წინადადებებით**. Laravel `message`-ში პირველივე შეცდომას წერს, ე.ი. კოდი პირდაპირ `errorMessage()`-ს ხვდება და ითარგმნება (BUG-14-ის შემდეგ ის `CODES`-ს ჯერ ამოწმებს)
  - ✅ რვავე კონტროლერი განახლდა; SPA-ს „ცარიელად დატოვება" ახლა `clear_records`-ს აგზავნის; ორივე კოდი `CODES`-შია და ორივე ლოკალში
  - ✅ **სამი არსებული ტესტი წითლდებოდა და ეს იყო მთელი აზრი** — სამივე ძველ ჩუმ ნაგულისხმევს ეყრდნობოდა. ახალი `test_deleting_without_an_explicit_intent_is_rejected` სამივე მცდარ ფორმას ამოწმებს და ბოლოს ადასტურებს, რომ **არც ერთ უარყოფას არაფერი არ შეუცვლია**
  - ✅ CLAUDE.md-ის 🟡 პუნქტი („one hole is left open") შესაბამისად გადაიწერა
- **ტიპი:** gap
...
- **Acceptance criteria:**
  - [x] გადაწყვეტილება CLAUDE.md-სა და docblock-ში ერთნაირადაა; (a)-ს ტესტი წერია
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — `frontend/scripts/error-codes.mjs`, `npm run lint`-ში. დღეს **82 კოდი**, ყველა რეგისტრირებული.
  - ✅ **ჯაჭვის პირველი რგოლი**: backend-ის კოდი ⊆ `CODES`. მეორე რგოლი (`CODES` ⊆ ორივე ლოკალი) უკვე `errors.test.ts`-შია და აქ **განზრახ არ მეორდება** — ერთი ფაქტი ერთ ადგილას
  - ⚠️ **სკრიპტი და არა Vitest-ის ტესტი**: `src`-ს `@types/node` განზრახ არ აქვს (ის ბრაუზერის კოდია და `process`/`fs` იქ არ უნდა ჩანდეს) — `node:fs`-ის მკითხველი ტესტი `tsc -b`-ს ატეხდა. პირველი ვერსია ზუსტად ამაზე ჩავარდა
  - ⚠️ **ხუთი ფორმა სკანირდება და ეს ემპირიულია**: `'message' => …`-ის grep 81-ს პოულობს, მაგრამ `csrf_token_mismatch`-ს **ვერ** — ის `new HttpException(419, '…')`-ია `bootstrap/app.php`-ში. დანარჩენი სამი: `abort_unless(…, 422, 'code')`, `ChatService::fail()`, `['reason' => …]`. სწორედ ამიტომ სკანირდება `bootstrap/` და `routes/`, და არა მარტო `app/`
  - ⚠️ „კოდი" **ფორმითაა** განსაზღვრული (`snake_case`, ორი სეგმენტი) და არა სიით — ვალიდატორის ინგლისური წინადადება ამ ფორმას არ აკმაყოფილებს, ე.ი. ხმაური არ ჩნდება
  - ⚠️ სკრიპტი **ჯერ საკუთარ წამკითხველებს ამოწმებს** (≥50 ნაპოვნი + ორი ღუზა, `CODES` ≥50): გადარქმეული საქაღალდე ან შეცვლილი ფორმა მას უხმოდ გაატარებდა ცარიელ სიაზე
  - ⚠️ `backend/`-ის არყოფნაზე შემოწმება **გამოტოვდება და არა ჩავარდება** — თორემ ცალკე გაშვებულ frontend-ზე `lint` სამუდამოდ წითელი იქნებოდა
  - ✅ **დადასტურდა ცოცხლად**: კონტროლერში ჩასმული `'brand_new_code'` → `npm run lint` **exit 1** და კოდს ფაილთან ერთად ასახელებს
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] კოდის დამატება `CODES`-ის გარეშე CI-ს აწითლებს (`npm run lint`); ლოკალის გარეშე — `errors.test.ts` (`npm test`)
- **Estimate:** S
- **დამოკიდებულება:** GAP-01

### [FEAT-02] `mediary:seed-demo` — ანონიმური საჩვენებელი მონაცემები კომიტებული dump-ის ნაცვლად
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — backend 791/791.
  - ✅ `mediary:seed-demo` — დემო-ანგარიში (`demo@example.com` / `demo-password`, `--email=`/`--password=`-ით შეცვლადი) + **14 სინთეზური ჩანაწერი** ათივე მოდულზე; `setup.sh`, `setup.ps1` და README მას ასახელებენ
  - ⚠️ **ყველა ჩანაწერი სინთეზურია**: ელფოსტა `example.com`-ზეა (RFC 2606), გარე წყაროს არც ერთი id არ ჩნდება — ე.ი. შედეგი, dump-ისგან განსხვავებით, პერსონალურ მონაცემს არ შეიცავს
  - ⚠️ **პროდუქციაზე დადასტურებას ითხოვს** (`ConfirmableTrait`): ბრძანება ანგარიშს **ცნობილი პაროლით** ქმნის, ე.ი. სერვერზე შემთხვევითი გაშვება უკანა კარს გააღებდა
  - ⚠️ **ჩანაწერები მოდელით იქმნება და არა API-ით**, ე.ი. `HasStatus`-ის `creating` ჰუკი სტატუსს თვითონ აყენებს და ლექსიკონები `ensureDefaults()`-ით ივსება. ცალკე ტესტი ამოწმებს, რომ შედეგი **სავალდებულო ველების წესს** აკმაყოფილებს (2026-09-16) — თორემ დემო ისეთ მდგომარეობას აჩვენებდა, რომელსაც აპი აღარ უშვებს
  - ⚠️ `Auth::setUser()` აქაც აუცილებელია (`RunBatchItem`-ის იგივე მიზეზი): CLI-ზე `owner` scope გამორთულია და ჩანაწერები `user_id`-ის გარეშე დაიბადებოდა
  - ⚠️ **ხელახლა გაშვება დუბლიკატებს არ ქმნის** (`already()` თითო მოდულზე) — ბრძანება სწორედ იმას სჭირდება, ვინც ახალ მოდულს ამატებს, ე.ი. ის მრავალჯერ გაეშვება
  - ⚠️ სათაური `setTranslation()`-ით იწერება: `title_ka` **accessor-ია და არა სვეტი** (`<domain>_translations`), ე.ი. მისი პირდაპირი მინიჭება უხმოდ არაფერს ჩაწერდა — პირველი ვერსია ზუსტად ასე იყო
  - ⚠️ **ხარვეზი, რომელიც აღიარებას იმსახურებს:** ბრძანება შემოწმებისას **ცოცხალ ბაზაზე** გაეშვა და იქ დემო-ანგარიში და 14 ჩანაწერი შექმნა. მაშინვე გასუფთავდა (`$demo->delete()`, კასკადებით) და დადასტურდა, რომ არაფერი დარჩა: 2 მომხმარებელი, 377 ფილმი, `statuses`/`video_types`/`song_genres`/`module_user` დემოზე — 0. ასეთი ბრძანება **მხოლოდ სატესტო ბაზაზე** უნდა გაეშვას
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] `php artisan migrate:fresh --seed && php artisan mediary:seed-demo` სრულ სამუშაო აპს იძლევა
  - [x] repo-ში პერსონალური მონაცემი არ არის (ჩანაწერები სინთეზურია, ელფოსტა `example.com`)
- **Estimate:** M
- **დამოკიდებულება:** SEC-01

### [FEAT-03] პარტიის თითო ერთეულის შედეგი (`ok`/`skipped`/`failed` + მიზეზი) და მისი ჩვენება SPA-ში
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — backend 787/787, frontend 172/172; მიგრაცია ცოცხალ ბაზაზეც.
  - ✅ `batch_items` (`batch_id`, `user_id`, `kind`, `type`, `record_id`, `status`, `title`, `error`), `RunBatchItem` წერს, `BatchController::payload()` აბრუნებს `items`-ს
  - ⚠️ **სამი მდგომარეობა და არა ორი**: `ok` · `skipped` (ჩანაწერი წაიშალა — **ხელახლა გაშვება არაფერს შეცვლის**) · `failed` (**ესაა ის, რაც გადასაშვებია**). ერთ დროშად შეკუმშვა სწორედ იმ კითხვას ტოვებდა პასუხგაუცემელი, რომლის გამოც ცხრილი არსებობს
  - ⚠️ **რიგი job-ის დასაწყისში იწერება** და არა ბოლოს: ჩავარდნაზე გამონაკლისი გადაისვრება (BUG-08), ე.ი. ბოლოში ჩაწერა **ჩავარდნილ ერთეულს საერთოდ არ დააფიქსირებდა**
  - ⚠️ **`run()` ახლა `bool`-ს აბრუნებს და ეს გასწორება ტესტმა აჩვენა**: უპირობო `OK` ახლახან დაწერილ `skipped`-ს გადააწერდა — სია იტყოდა, რომ წაშლილი ჩანაწერი დამუშავდა
  - ⚠️ სათაური **გაშვების მომენტში** იწერება: ჩანაწერი მოგვიანებით შეიძლება წაიშალოს, სიაში კი „რა იყო ეს" უნდა დარჩეს
  - ⚠️ `batch_id` **სტრიქონია და უცხო გასაღები არ არის**: `job_batches.id` Laravel-ის შიდა ცხრილია და მასზე FK ჩვენს სქემას ფრეიმვორკის დეტალზე მიაბამდა
  - ⚠️ `error`-ში **მხოლოდ შეტყობინება** ჯდება (500 სიმბოლომდე): stack trace `sources.log`-შია და მისი UI-ში გამოტანა შიდა ბილიკებს გაამხელდა
  - ✅ **SPA**: გადაცემის შემდეგ პარტია **იდევნება** (3 წმ poll, მხოლოდ სანამ მიმდინარეობს) და შედეგები იმავე პანელში, **იმავე მარკირებით** იხატება — ორ რეჟიმს ერთი ვიზუალური ენა აქვს. აქამდე გადაცემა „გაუშვი და დაივიწყე" იყო: toast ამბობდა „N გადაეცა" და მერე არაფერი
  - ⚠️ სერვერული ერთეულები `items`-ში **არ ერევა**: იქ ისინი `isBusy`-ს, გაჩერებასა და ხელახლა ცდას ჩაებმებოდნენ — ვერც ერთი ვერ მოქმედებს გადაცემულ სამუშაოზე. `isBusy` კი **ითვლის** მათ, თორემ პანელი დასრულებულად გამოიყურებოდა worker-ის მუშაობისას
  - ✅ **DEBT-11-ის მცველმა ახალი მოდელი მაშინვე დაიჭირა** — `BatchItem` არც `MODELS`-ში იყო და არც `NOT_LOGGED`-ში; ჩაიწერა მიზეზით (მიწოდების ჟურნალია, 300-ერთეულიანი გაშვება ლოგს დამარხავდა). ე.ი. იმავე დღეს დაწერილმა ტესტმა თავისი საქმე გააკეთა
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] პარტიის დასრულებაზე თითო ერთეულის სტატუსი API-შიც და UI-შიც ჩანს
- **Estimate:** M
- **დამოკიდებულება:** BUG-08

### [FEAT-04] ალბომის პაროლის წარუმატებელი ცდების DB-მრიცხველი და დროებითი დაბლოკვა
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — backend 779/779; მიგრაცია ცოცხალ ბაზაზეც გაშვებულია.
  - ✅ `gallery_albums.failed_unlocks` + `unlock_blocked_until`; **10 არასწორი ცდა → 15 წუთით ბლოკი**, `423 album_temporarily_locked` (`CODES`-შია და ორივე ლოკალში)
  - ⚠️ **სვეტს `unlock_blocked_until` ჰქვია და არა `locked_until`**: „locked" ამ ცხრილში უკვე **პაროლის არსებობას** ნიშნავს (`isLocked()`), ე.ი. მეორე მნიშვნელობა ერთ სიტყვაზე ორ ფაქტს დაადებდა
  - ⚠️ **აღრიცხვა ერთ ადგილას** — `GalleryAlbum::registerUnlockAttempt()` — და **ორივე კარი მას ეძახის**: შიდაც (`/gallery/albums/{id}/unlock`) და საჯაროც. უამისოდ თავდამსხმელი უბრალოდ მეორე endpoint-ზე გადავიდოდა
  - ⚠️ **ბლოკის შემოწმება `Hash::check`-ზე ადრეა** (BUG-02-ის იგივე რიგი): შემდეგ დაბრუნებული 423 ორაკულს ადგილზე დატოვებდა, უბრალოდ სხვა კოდით. ტესტი `Hash::shouldNotHaveReceived('check')`-ს ამოწმებს და არა მხოლოდ სტატუსს
  - ⚠️ **სწორი პაროლიც 423-ია ბლოკის დროს** — ესაა მთელი აზრი; უამისოდ გამოცნობა უბრალოდ შენელდებოდა
  - ⚠️ ბლოკის დადგმისას მრიცხველი **ნულდება**, თორემ ერთხელ დაბლოკილი ალბომი მე-11 ცდიდან სამუდამოდ დაბლოკილი დარჩებოდა; სწორი პაროლიც ანულებს — თორემ დროთა განმავლობაში ლეგიტიმური მფლობელიც დაიბლოკებოდა
  - ⚠️ `saveQuietly()`: ეს ტექნიკური მრიცხველია — `AuditObserver` მას ყოველ არასწორ პაროლზე ლოგში ჩაწერდა და ჟურნალს დამარხავდა
  - ⚠️ **მრიცხველი მხოლოდ მფლობელის სიაშია** (`GalleryAlbumController::row()`), საჯარო პასუხში — არა: იქ ის თავდამსხმელს ეტყოდა, რამდენი ცდა დარჩა და როდის გაიხსნება, ე.ი. თვითონ მცველი გახდებოდა მინიშნება
  - ⚠️ ტესტში `throttle` **განზრახაა გამორთული**: `throttle:album-unlock` 10/წთ-ია, ე.ი. ერთი IP-დან DB-მრიცხველამდე მისვლა შეუძლებელია — და სწორედ ესაა მისი აზრი, მრიცხველი **IP-ის როტაციისთვის** არსებობს
  - ✅ **მუტაცია:** ბლოკის შემოწმება `Hash::check`-ის შემდეგ გადავიდა → ტესტი წითლდება („check … called 1 times")
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] 10 არასწორი პაროლი ალბომს ბლოკავს (IP-ის მიუხედავად — მრიცხველი ალბომზეა); სწორი პაროლი დაბლოკვის დროს 423-ია
- **Estimate:** S
- **დამოკიდებულება:** BUG-02

### [FEAT-05] `mediary:doctor` — ბინარების, scheduler-ის და storage-მრიცხველის დრიფტის ერთი შემოწმება
- **სტატუსი:** ✅ შესრულებულია (2026-09-18) — backend 785/785; **ცოცხალი გაშვება ამ მანქანაზე ნამდვილ ხარვეზს პოულობს**.
  - ✅ ოთხი ბლოკი: **ბინარები** (`yt-dlp` · `ffmpeg` · `mysqldump` · `mysql`) · **scheduler** · **საცავის მრიცხველი** (თითო ანგარიშზე) · **პარამეტრები** (`APP_DEBUG`, `SESSION_SECURE_COOKIE`, `PUBLIC_PROFILES`, `TMDB_API_KEY`). პრობლემაზე — არა-ნულოვანი კოდი
  - ⚠️ **სამი დონე და არა ორი.** `WARN` „პატარა შეცდომა" არაა: `ffmpeg`-ის არქონა ლეგიტიმური კონფიგურაციაა (ჩამოწერა მუშაობს, უბრალოდ ერთფაილიან ხარისხზე), scheduler-ის არყოფნა შეხსენებებს **არ კარგავს** (ბრაუზერის polling იმავე დისპეტჩერს იძახებს, §8.2), `APP_DEBUG=true` კი ლოკალურად **სწორია**. მათი FAIL-ად ჩათვლა ბრძანებას მუდმივად წითელს გახდიდა და პირველივე კვირაში გამორთვამდე მიიყვანდა
  - ⚠️ **„dry-run" ნამდვილად dry-ია**: `recalculate()` მრიცხველს **წერს**, ე.ი. მისი გამოძახება დრიფტს დაფარავდა იმავე წამს, როცა აღმოაჩენდა. `doctor` `files()`-ის ჯამს ცალკე ითვლის და მხოლოდ ადარებს — ცალკე ტესტი ამოწმებს, რომ მრიცხველი **არ გადაწერილა**
  - ⚠️ **გასაღები არასდროს იბეჭდება** — მხოლოდ „ჩაწერილია/ცარიელია"; დიაგნოსტიკის გამონატანი ლოგებში, ეკრანის სურათებსა და CI-ს არტეფაქტებში ხვდება. ცალკე ტესტი `doesntExpectOutputToContain`-ით
  - ⚠️ **scheduler-ის ნიშანი არ არსებობდა და დაემატა**: `notes:remind` ახლა ქეშში წერს ბოლო გაშვების დროს (`SendNoteRemindersCommand::HEARTBEAT`). უამისოდ „შეხსენება არ მოვიდა" შეიძლება იმიტომაც ყოფილიყო, რომ ვადამოსული უბრალოდ არ იყო. ქეშში და არა ცხრილში: ეს დიაგნოსტიკაა, დაკარგვა „არ ვიცი"-ს ნიშნავს და არა არასწორ პასუხს
  - ⚠️ `environment(['local', 'testing'])` და არა მხოლოდ `local`: phpunit `APP_DEBUG=true`-ით გადის, ე.ი. უამისოდ **ბრძანების საკუთარი ტესტი** იქნებოდა წითელი
  - ⚠️ ტესტი ბინარებს **მოკავს**: ნამდვილი `yt-dlp` მანქანაზე იყოს თუ არა, ლოგიკის საქმე არაა — თორემ ტესტი გარემოს მიჰყვებოდა და CI-ზე სხვა პასუხს გასცემდა
  - ⚠️ `mb_str_pad` და არა `str_pad`: ქართული სახელი (ელფოსტის ნაცვლად) სვეტს არღვევდა
  - 🟡 **ცოცხალმა გაშვებამ ამ მანქანაზე იპოვა**: `yt-dlp` — **FAIL**, `ffmpeg` — WARN. ორივე `.env`-ში **ცარიელია** და PATH-ზეც არაა, ე.ი. ვიდეოს ჩამოწერა აქ 503-ია. CLAUDE.md-ის ჩანაწერი მათი დაყენების შესახებ **სხვა მანქანას ეხება** (2026-09-14); ეს დესკტოპი 2026-09-17-დან მუშაობს. დაყენება შენი გადაწყვეტილებაა (`pip install --user yt-dlp` + gyan.dev-ის ffmpeg, შემდეგ ბილიკები `.env`-ში) — ე.ი. ბრძანებამ ზუსტად ის იპოვა, რისთვისაც დაიწერა
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] `php artisan mediary:doctor` ყველა პუნქტზე OK/WARN/FAIL-ს ბეჭდავს და პრობლემაზე არა-ნულოვან კოდს აბრუნებს
- **Estimate:** M
