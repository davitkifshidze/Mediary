...
- **Acceptance criteria:**
  - [ ] `grep -rn "[ა-ჰ]" backend/app --include=*.php | grep "'message' =>\|withMessages\|Exception('"` ცარიელია
  - [ ] `npm run check:codes` (error-codes.mjs) წინადადებას `message`-ში წითლად აღნიშნავს
  - [ ] ქართულ UI-ზე `required` ვალიდაცია ქართულად მოდის (ტესტი `Accept-Language: ka`-თი)
- **Estimate:** M
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] `grep -c "ჟანრიის\|კატეგორიაის\|ნიშავს\|სასაათე\|გალერიის\|დამრჩეს\|ფრენჩაიზ\|ნაცვლად არ არის" frontend/src/i18n/ka.json` → 0
  - [ ] ხუთივე `*.moveTo` სახელობით მრავლობითშია
  - [ ] i18n audit მწვანეა და გასაღებების რაოდენობა ორივე ლოკალში უცვლელია (2690)
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] ტესტი: მომხმარებელს თითო მოდულზე თითო ატვირთვა აქვს → `DELETE /admin/users/{id}`-ის შემდეგ `Storage::disk()->exists()` ყველაზე false-ია და `storage/orphans` 0-ს აბრუნებს
  - [ ] მეორე მონაწილის `GET /chat` ghost-საუბარს არ აბრუნებს
- **Estimate:** M
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] `grep -n "\.env\|UTC\|უკატეგორიო" frontend/src/i18n/ka.json` მხოლოდ იმ ხაზებს აბრუნებს, სადაც ფაქტი ისევ სწორია (`credentials.installationHint`, `backups.unavailableHint`)
  - [ ] i18n audit მწვანეა
- **Estimate:** S
...
- **Acceptance criteria:**
  - [ ] გლოსარი დაწერილია და თითო ცნებას ერთი სიტყვა აქვს
  - [ ] `grep -c "ლინკ\|თამბნეილ\|კლავიშ\|ფრენჩაიზ" frontend/src/i18n/ka.json` → 0
  - [ ] i18n audit-ის ახალი შემოწმება აკრძალულ სიტყვას წითლად აღნიშნავს
- **Estimate:** M
- **დამოკიდებულება:** BUG-18
...
- **Acceptance criteria:**
  - [ ] ტესტი: `ready` ვიდეო + კვოტა სავსე → `POST /videos/{id}/download` 413-ია და `download_path`/ფაილი ადგილზეა
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] ტესტი: `enrichMovie()` ახალ ჟანრზე `name_ka`-ს არ წერს და `TranslationScanner::genreMissing()` `name_ka`-ს აბრუნებს
  - [ ] ცოცხალ ბაზაზე `genre_translations`-ში ka რიგი მხედრულის გარეშე 0-ია
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] ტესტი: `GET /matches` 5 და 50 საჯარო პროფილზე `information_schema`/`PRAGMA` query-ების რაოდენობა ერთნაირია
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] ტესტი: `GET /chat` 3 და 30 საუბარზე query-ების რაოდენობა ერთნაირია
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] `GET /translations/summary` ჩანაწერებს არ ტვირთავს (query-ების რაოდენობა ბიბლიოთეკის ზომაზე არაა დამოკიდებული)
  - [ ] რიცხვები `plan()`-ის სიას ემთხვევა (არსებული `TranslationTest` ინარჩუნებს)
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] ტესტი (MySQL-ზე, `@requires`): ასლის წაშლა `_inspect_` ბაზას აგდებს
  - [ ] `doctor` ღია ვიუერს WARN-ით აჩვენებს
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] სამივე ხაზი შენობითშია; i18n audit მწვანეა
- **Estimate:** S
- **დამოკიდებულება:** BUG-18
...
- **Acceptance criteria:**
  - [ ] `grep -c '„[^"“]*"' frontend/src/i18n/ka.json` → 0
- **Estimate:** S
- **დამოკიდებულება:** BUG-18
...
- **Acceptance criteria:**
  - [ ] ხუთივე ხაზი ქართულადაა; allow-სიის გარეთ ლათინური სიტყვა ka.json-ში 0
- **Estimate:** S
...
- **Acceptance criteria:**
  - [ ] `/does-not-exist` 404 გვერდს აჩვენებს და ბმულით `/`-ზე გადადის
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] `audit.py` „unused" სექციას ბეჭდავს და გასუფთავების შემდეგ 0-ს აჩვენებს
  - [ ] ორივე ლოკალში გასაღებების რაოდენობა ერთნაირია და აპი ყველა გვერდზე ტექსტს პოულობს (`npm test` + i18n audit მწვანე)
- **Estimate:** S
...
- **Acceptance criteria:**
  - [ ] ტესტი: 100-სიმბოლოიანი ქართული სახელით ორი ჟანრი უნიკალურ, სვეტში მოთავსებულ slug-ს იღებს
- **Estimate:** S
- **დამოკიდებულება:** none
...
- **Acceptance criteria:**
  - [ ] ტესტი `Http::fake()`-ის 5 MB სხეულზე: მეხსიერებაში ≤2 MB იკითხება (ან პასუხი `MAX_BYTES`-ზე წყდება)
- **Estimate:** S
- **დამოკიდებულება:** none
