...
- **სტატუსი:** ✅ შესრულებულია (2026-09-19). `GET /api/export` (რა მაქვს) + `GET /api/export/{module}?format=json|csv`, რეესტრი **`App\Support\ExportDomain`**, აგება **`Services\Export\RecordExporter`**, ბლოკი „ჩემი მონაცემები" `/profile`-ზე (`components/DataExportCard.tsx`) — საცავის ბარათის ქვემოთ, რადგან ზემოთ **ფაილების** არქივია და აქ **ჩანაწერების** სია.
  - ⚠️ **`PublicDomain::card()` ვერ გამოდგა და ეს ორმხრივია**: ის განზრახ **ნაკლებს** ამბობს (უცხო თვალისთვისაა), ექსპორტს კი სისრულე სჭირდება; და პირიქით — `note` `PublicDomain`-ში **საერთოდ არ არის** (§16.5), ექსპორტში კი სწორედ ის არის ერთ-ერთი ყველაზე საჭირო.
  - ⚠️ **`toArray()` არ გამოიყენება**: მასში `user_id`/`status_id`/`sync_status` ხვდება, CSV-ს კი სვეტების **სტაბილური რიგი** სჭირდება — ფორმატი, რომელიც მიგრაციაზე თვითონ იცვლება, ფორმატი არ არის (FEAT-07 სწორედ მას დაეყრდნობა).
  - ⚠️ **`status` და `status_name` ორივე გადის**: გასაღები არასდროს იცვლება (იმპორტი მას დაამთხვევს), სახელი კი წასაკითხია და ლექსიკონში ნებისმიერ დღეს შეიძლება გადაირქვას.
  - ⚠️ **ცხადი `user_id` და არა `owner` scope**: scope `Auth::id()`-ს კითხულობს, ე.ი. worker-იდან **ყველა** ანგარიშის ჩანაწერი მოხვდებოდა ერთ ფაილში (`RunBatchItem`-ის ზუსტი გაკვეთილი).
  - ⚠️ **CSV: BOM + ფორმულის ესკეიპინგი.** უამისოდ Excel მხედრულს ჯღაბნად ხსნის; `=`/`+`/`@`-ით დაწყებული უჯრა კი **ფორმულაა**, ხოლო ტექსტი გარედან მოდის (ბუკმარკის სათაური `LinkMetadata`-ს უცხო გვერდიდან). უარყოფითი რიცხვი განზრახ არ ესკეიპდება — `-5` ტექსტად იქცეოდა.
  - ⚠️ **`AuditLog::ACTION_EXPORT`** ცალკე მოქმედებაა (`view`/`visit` მთელი მოდულის სერვერიდან გატანას ვერ აღწერს) და ლოგი **ნაკადის დაწყებამდე** იწერება — ჰედერების გაგზავნის შემდეგ ჩაწერის ჩავარდნა ვერსად გამოჩნდებოდა.
  - ⚠️ `RegistryConsistencyTest`-ის ორი ახალი შემოწმება: ყოველი ჩანაწერიანი მოდული ან `MODULES`-შია, ან `NOT_EXPORTED`-ში (მიზეზით — `gallery` იქაა, მისი შიგთავსი **ფაილებია**), და ყოველი ველი მართლა სვეტია/აქსესორია/ვირტუალურია — ბეჭდვითი შეცდომა (`titel_ka`) ჩუმად ცარიელ სვეტს ტოვებდა (გადამოწმდა: შეცდომის შეტანაზე ტესტი წითლდება).
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] თითო მოდულზე CSV/JSON მხოლოდ საკუთარ ჩანაწერებს შეიცავს (ტესტი სხვისი ჩანაწერით) — `ExportTest::test_the_export_contains_only_my_own_records`
  - [x] ექსპორტი აუდიტ-ლოგში იწერება — `ExportTest::test_the_export_is_written_to_the_audit_log`
- **Estimate:** M
- **დამოკიდებულება:** none

### [FEAT-07] იმპორტი გარე სერვისების CSV-დან
- **სტატუსი:** ✅ შესრულებულია (2026-09-19). `GET /api/import/sources` · `POST /api/import/plan` (multipart) · `POST /api/import/item`; რეესტრი **`App\Support\ImportSource`**, წაკითხვა **`Services\Import\CsvReader`**, გეგმა **`ImportPlanner`**, შექმნა **`RowImporter`**; გვერდი **`/import`** (სამი ნაბიჯი) + რიგის ახალი სახეობა `import`.
  - ⚠️ **გეგმა გარე წყაროს არ ეკითხება** — 800-რიგიანი ფაილი 800 TMDB-გამოძახება იქნებოდა **მხოლოდ რიცხვის საჩვენებლად**, და მერე კიდევ იმდენივე იმპორტზე. გეგმა მხოლოდ იმას აკეთებს, რაც უფასოა: კითხულობს, სვეტებს ალაგებს და **ადგილობრივ** დუბლს ეძებს; გარე ძებნა ერთეულის ბიჯზეა, სადაც `syncDelayMs`-ის პაუზაც მოქმედებს.
  - ⚠️ **ფაილი არსად არ ინახება** — გეგმა სრულ, უკვე ნორმალიზებულ სიას აბრუნებს პასუხში, ე.ი. ერთეულის ბიჯს ფაილი აღარ სჭირდება: არც დროებითი საქაღალდე, არც კვოტა, არც გასუფთავების ვადა.
  - ⚠️ **BOM ხელით იჭრება.** Excel-ში შენახულ CSV-ს პირველი სვეტი `\u{FEFF}Date` ჰქვია — ე.ი. **ხელმოწერის ამოცნობა და სვეტების რუკა ჩუმად ცდებოდა** სწორედ იმ ფაილებზე, რომლებსაც ყველაზე ხშირად ტვირთავენ. ფიქსტურა (`tests/Fixtures/letterboxd-watched.csv`) განზრახ BOM-ით იწყება და ამას ცალკე ტესტიც ამოწმებს.
  - ⚠️ **გამყოფი ცნობადია** (`,` · `;` · tab · `|`): ევროპულ ლოკალზე Excel `;`-ით ინახავს და არასწორი გამყოფი **ერთსვეტიან ცხრილს** იძლევა, ე.ი. „ფაილი ცარიელია"-ს ტიპის უაზრო შეცდომას.
  - ⚠️ **Goodreads ISBN-ს `="9780…"`-ად წერს** (Excel-ის ხრიკი) — გაწმენდის გარეშე ზუსტი ძებნა **არასდროს მუშაობდა** და ყველა წიგნი „ახალი" იქნებოდა.
  - ⚠️ **შეფასების `0` „შეფასების გარეშეა" და არა „ყველაზე ცუდი"** — პირდაპირი გადმოწერა მთელ ბიბლიოთეკას ერთვარსკვლავიანად აქცევდა; Letterboxd-ის 0.5–5 კი ×2-ით გადადის ჩვენს ათბალიანზე.
  - ⚠️ **სტატუსი `role`-ით ისმება და არა სახელით** (§6.4): ჩემი „ნანახი" და შენი „ვნახე" ერთი და იგივეა, გასაღები კი სხვა — ტესტი გადარქმეულ ლექსიკონზე ამოწმებს.
  - ⚠️ **დუბლი ორჯერ მოწმდება**: გეგმაში (ადგილობრივად, `imdb_id`/`isbn`, მერე სათაური+წელი) და **ჩაწერისას თავიდან** — `tmdb_id` მხოლოდ ძებნის შემდეგ ხდება ცნობილი, ე.ი. „უკვე მაქვს" მხოლოდ სერვერის სიტყვაა (`/purge`-ის `plan()`/`run()`-ის იგივე წესი).
  - ⚠️ **ჟანრი/კატეგორია არ ივსება და ეს შეგნებულია**: „სტატუსი და ტიპი სავალდებულოა" **ფორმის** წესია და არა სქემის; `POST /{domain}/from-tmdb` ზუსტად ასევე უვლის მას გვერდს.
  - ⚠️ **უცნობი ფორმატი 422-ია და არა ცარიელი გეგმა** (`import_source_unknown`, `lib/errors.ts`-ში რეგისტრირებული) — ცარიელი სია ეკრანზე „ფაილი ცარიელია"-დ იკითხება; პასუხში სათაურები მოდის, რომ წყარო ხელით აირჩიო.
  - ⚠️ **ეს „ლინკების ბოტი" არ არის** (2026-09-05-ს სამუდამოდ მოხსნილი): იქ აპი თვითონ დაეძებდა ბმულებს უცხო საიტებზე, აქ კი მომხმარებელი საკუთარ ფაილს ტვირთავს.
  - ⚠️ **გვერდითა მენიუს რიგი საკმარისი არ აღმოჩნდა — ექსპორტის ბარათიდანაც ბმულია (2026-09-19, შენი შენიშვნით „იმპორტი ვერ მივაგენი სად არის").** `/import` და მენიუს რიგი თავიდანვე არსებობდა; პრობლემა ისაა, რომ **ექსპორტი `/profile`-ზეა**, ე.ი. ერთი კითხვის ორი ნახევარი ორ სხვადასხვა ადგილას იდგა და მეორეს თხუთმეტრიგიან ხელსაწყოების სიაში ეძებდი. ⚠️ **ბმულია და არა მეორე ფორმა**: იმპორტი სამნაბიჯიანია და ბარათში ვერ ჩაჯდებოდა, ორი ადგილი კი, სადაც ფაილს ირჩევ, ორ სხვადასხვა ქცევად იკითხებოდა.
  - ℹ️ **Steam-ის `appid`-ით პირდაპირი ძებნა RAWG-ის უფასო API-ს არ აქვს**, ამიტომ თამაში სახელით იძებნება; `appid` მაინც მოგვაქვს, რადგან ის ერთადერთია, რაც ორ ერთსახელა რიგს გამოარჩევს.
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] Letterboxd-ის სატესტო CSV 10 ფილმს ქმნის, დუბლს გამოტოვებს და ანგარიშს აჩვენებს — `ImportTest::test_ten_films_are_created_and_a_duplicate_is_skipped`
- **Estimate:** L
- **დამოკიდებულება:** FEAT-06

### [FEAT-08] სტატისტიკის გვერდი
- **სტატუსი:** ✅ შესრულებულია (2026-09-19). `GET /api/stats?year=` (ერთი რექვესთი ყველა ჩართულ მოდულზე), სერვისი **`Services\Stats\LibraryStats`**, დრაივერის სხვაობა **`App\Support\SqlDate`**-ში, გვერდი **`/stats`** — ხუთი ჭრილი თითო მოდულზე: სტატუსი · თვეები · ჟანრი · ქულა · გამოშვების წელი.
  - ⚠️ **დეშბორდს არ ცვლის და 19.10-ის გადაწყვეტილებას არ აუქმებს**: ბარათი „რა მაქვს"-ს პასუხობს (ერთი რიცხვი + ბმული), ეს გვერდი — „რა გავაკეთე". ექვსი რიცხვი ბარათზე ნავიგაციას გააფუჭებდა.
  - ⚠️ **ზოლები ნამდვილ გრაფიკებად შეიცვალა (2026-09-19, შენი მითითებით — „ეს უბრალოდ ზოლები არ მომწონს, გამოიყენე ბიბლიოთეკები").** პირველი გადაწყვეტილება („ბიბლიოთეკა არ დამატებულა, ყველა ჭრილი ერთი `div`-ია") ორ რამეს არ ითვალისწინებდა: `div`-ს **ღერძი და ტულტიპი არ აქვს** (თორმეტი თვე ზოლებად არ იკითხება — ვერ ხედავ *როდის*), და ხუთივე ჭრილი ერთ ფორმად იყო დაყვანილი, მაშინ როცა მათი ამოცანები სხვადასხვაა. ბიბლიოთეკა **recharts**-ია და გადამწყვეტი არგუმენტი ისაა, რომ **SVG-ია და არა canvas**: აპის თემა CSS-ცვლადებია და SVG-ის `fill` მათ პირდაპირ კითხულობს, ე.ი. ბნელი თემა უფასოდ მუშაობს; canvas-ს ფერი **რიცხვად** სჭირდება და გადართვაზე JS-ით ხელახლა დახატვა, რაზეც `useTheme()`-ს ამ პროექტში უკვე ეთქვა უარი.
  - ⚠️ **თითო ჭრილს თავისი ფორმა აქვს**: სტატუსი — **დონატი** (ნაწილი მთელთან, ≤6 სექტორი; მეტზე ზოლები, ერთზე უბრალოდ რიცხვი) · თვეები — **ფართობიანი ხაზი** (დრო) · ჟანრები — **ჰორიზონტალური ზოლები** (სიდიდე გრძელ სახელებზე) · ქულა და გამოშვების წელი — **სვეტები** (განაწილება რიგობრივ ღერძზე).
  - ⚠️ **ღირებულება ცალკე ჩანქშია.** recharts ≈104 kB gzip-ია და დეშბორდი სადესანტო გვერდია, ამიტომ `DashboardStats` `lazy()`-ა **ლოკალური `Suspense`-ით** (მის გარეშე მთელი გვერდი `App.tsx`-ის ფოლბექზე ჩავარდებოდა). გაზომილი: entry უცვლელი — 122.71 kB gzip.
  - ⚠️ **დეშბორდზე ოთხი მთავარი რიცხვი დაემატა** (`GET /api/stats/summary`): სულ ჩანაწერი · რჩეული · წელს დასრულებული · ამ თვეში. ⚠️ **ცალკე endpoint-ია** — სრული პასუხი ~50 query-ა და მთავარი გვერდი მას ვერ დაელოდებოდა; **წელს არ იღებს**, რადგან არჩეულ 2019-ში „ამ თვეს" აზრი არ აქვს. ⚠️ **გალერეა ჯამში არ ითვლება** (მისი შიგთავსი ფაილებია — `ExportDomain`-ის იგივე წესი), ე.ი. ჯამი ბარათების ჯამს შეიძლება დააკლდეს.
  - ⚠️ **`year()`/`strftime()` ორ დრაივერზე სხვადასხვაა** და `whereYear()` ამას **არ ფარავს** — Laravel-ს ფილტრზე აქვს აბსტრაქცია, **დაჯგუფებაზე** კი არა. ერთი ვარიანტის ჩაწერა MySQL-ზე იმუშავებდა და sqlite-ზე ჩუმად **ცარიელ ჯგუფს** დააბრუნებდა (გრაფიკი ნულებით დაიხატებოდა, შეცდომის გარეშე). `SqlDate` სტრიქონს აბრუნებს და არა `Expression`-ს, რადგან `pluck(Expression)` Laravel-ში **სვეტის სახელად** იკითხება.
  - ⚠️ **თითო ჭრილი ახალ query-ს იღებს (`$base` ფაბრიკა).** `select('id')` და `groupBy` builder-ს **ადგილზე** ცვლიან, ე.ი. გაზიარებული ობიექტი მეორე ჭრილს ჩუმად აფუჭებდა — სწორედ ამით ბრუნდებოდა წლების სია ცარიელი პირველ გაშვებაზე.
  - ⚠️ **სტატუსი ორ მექანიზმზე დგას** (§6.4): ლექსიკონიდან სახელიც იკითხება (მფლობელისაა, `id` გვერდს ვერაფერს ეტყოდა), enum-იან სამ მოდულზე კი გასაღები პირდაპირ მოდის და სახელი `ka.json`-შია.
  - ⚠️ **პოლიმორფული ჟანრი `genreable_type`-ით უნდა იჭრებოდეს** — `genreables` ერთი ცხრილია სამივე მედია-დომენზე, ე.ი. ფილტრის გარეშე სერიალის ჟანრები ფილმების ჭრილში აღმოჩნდებოდა (პინდება ტესტით).
  - ⚠️ **„სხვა" რიგი ჯამს ინარჩუნებს**: ტოპ-12-ის მიღმა დარჩენილი ჟანრები ერთ რიგში იყრება, თორემ ზოლების ჯამი მთლიან რიცხვს აღარ დაემთხვეოდა.
  - ⚠️ **`done_at` ყველგან არ არსებობს და ეს არ არის გამორჩენა**: წიგნს „როდის წავიკითხე" სვეტად არ აქვს. `updated_at`-ის გამოყენება ორ სხვადასხვა ფაქტს ერთ სვეტში შეურევდა — ის ხაფანგი, რომლის გამოც ვიდეოს `watched_at` სტატუსს არ ეხმიანება.
  - ⚠️ **წლის ამომრჩევი აქტივობის წლებს აჩვენებს და არა გამოშვებისას** — „2011 წლის ფილმი" და „2011-ში ვნახე" ორი სხვადასხვა ფაქტია.
  - ⚠️ **თორმეტივე თვე ბრუნდება, ნულებიანიც** — თორემ წელი არათანაბრად დაიჭიმებოდა და „ივლისში არაფერი" იმ თვისგან ვერ განირჩეოდა, რომელიც სიაში არ იყო.
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] გვერდი თითო ჩართულ მოდულზე მინიმუმ ორ ჭრილს აჩვენებს და ცარიელ ბიბლიოთეკაზე `EmptyState`-ს — `StatsTest` (9 ტესტი; ცარიელი ბიბლიოთეკა — `test_an_empty_library_answers_with_zeroes`)
- **Estimate:** M
- **დამოკიდებულება:** none

### [FEAT-09] სერიალის სეზონების/ეპიზოდების პროგრესი
- **სტატუსი:** ✅ შესრულებულია (2026-09-19). ორი ცხრილი (`tv_episodes` + `episode_watches`), `TmdbClient::tvSeason()`, სერვისები **`Services\Episodes\EpisodeSync`** (ჩამოტანა) და **`EpisodeProgress`** (პროგრესი · შემდეგი · სტატუსი), `GET|POST|PATCH /api/media/episodes/{type}/{id}` და ბლოკი `components/EpisodeTracker.tsx` ჩანაწერის გვერდზე.
  - ⚠️ **ცხრილი `tv_episodes` ჰქვია და არა `series_episodes` (ტასქის სახელი), რადგან ის ანიმესაც ემსახურება**: `animes.tmdb_id` ასევე TMDB-ის *tv* id-ია (`TvEnricher` ორივეს `/tv/*`-ით ავსებს). `series_id`-ზე მიბმა ორ თითქმის იდენტურ ცხრილს დაბადებდა.
  - ⚠️ **გაყოფა შინაარსობრივია**: `tv_episodes` **TMDB-ის ფაქტია** (`user_id` არ აქვს — `genres`/`cast_members`-ის რიგში), `episode_watches` კი ჩემი. ერთ შოუს ას ანგარიშზე ას ასლს რომ ვინახავდეთ, ეს ხუთასი ეპიზოდი × ასი იქნებოდა.
  - ⚠️ **„ნანახია" რიგის არსებობაა და არა `is_watched` სვეტი** — ორი წყარო ერთი ფაქტისა ზუსტად ის ხაფანგია, რომელსაც `Book::syncProgress()` ებრძვის.
  - ⚠️ **ჩამოტანა ცხადი ღილაკია და არა სინქრონის ნაწილი**: ეპიზოდების სია **სეზონზე ერთი** გამოძახებაა (`append_to_response` მას ვერ მოიტანს), ე.ი. ჩვეულებრივ სინქრონში ჩაშენება მთელ ბიბლიოთეკაზე ასეულ რექვესთს ნიშნავდა.
  - ⚠️ **`season_number = 0` არ ჩამოიტვირთება** — TMDB-ზე ეს „სპეციალური გამოშვებებია" და მათი ჩათვლა სრულად ნანახ სერიალს 100%-ზე ნაკლებზე დატოვებდა.
  - ⚠️ **2026-09-19-ის შეცდომა („Table 'mediary.tv_episodes' doesn't exist") კოდის და არა ფუნქციის პრობლემა იყო: მიგრაციები არ იყო გაშვებული.** ხუთი მიგრაცია ელოდებოდა (`tv_episodes`-ისა და `next_air_at`-ის ჩათვლით); `php artisan migrate` გაშვებულია. ⚠️ **გაკვეთილი:** სიმპტომი ორ სხვადასხვა ფუნქციაზე გამოჩნდა (აქ — ნედლი SQL-შეცდომა, FEAT-10-ზე — უხმო სიცარიელე) და არცერთი არ ამბობდა ნამდვილ მიზეზს. ამიტომ `php artisan mediary:doctor` ახლა **ბაზის** სექციით იწყება და გაუშვებელ მიგრაციებს **FAIL**-ად ასახელებს.
  - ⚠️ **სტატუსი მხოლოდ წინ მიდის** (`todo` → `doing` → `done`): „დავასრულე და თავიდან ვუყურებ" კანონიერი მდგომარეობაა და ავტომატური დაწევა მას ჩუმად გააუქმებდა. კრიტერიუმი **როლია** და არა სახელი (§6.4).
  - ⚠️ **„შემდეგი" პირველი უნახავია და არა ბოლო ნანახის მომდევნო** — შუაში გამოტოვებული (ან მოგვიანებით დამატებული) ეპიზოდი სხვაგვარად სამუდამოდ დაიმარხებოდა.
  - ⚠️ **სეზონის მონიშვნაზე id-ების სიას სერვერი აგებს** — ბრაუზერს მხოლოდ ჩატვირთული გვერდი აქვს, სეზონი კი სრული უნდა იყოს.
  - ⚠️ **ჩაწერა `upsert()`-ია** (ერთი query სეზონზე) და არა ციკლში `updateOrInsert()`, რომელიც `created_at`-საც გადაწერდა ყოველ სინქრონზე.
  - ⚠️ **`RegistryConsistencyTest`-მა ორივე ახალი მოდელი დაიჭირა** — ყოველი მოდელი ან `AuditRegistry::MODELS`-შია, ან `NOT_LOGGED`-ში **მიზეზით**. ორივე მეორეშია: ეპიზოდის მონიშვნა ერთი დაწკაპუნებაა (სეზონზე კი ოცი ერთდროულად — რეაქციების იგივე მიზეზი), `TvEpisode` კი TMDB-ის ფაქტია და არა ადამიანის ქმედება.
  - ⚠️ **`still_path` განზრახ არ არსებობს** — `RegistryConsistencyTest` ყოველ `*path*` სვეტს `StorageMeter`-ის რუკაში ეძებს, აკორდეონს კი სურათი არ სჭირდება: ერთი სვეტი ნაკლები — ერთი გამონაკლისით ნაკლები.
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] ეპიზოდის მონიშვნა ინახება და სხვა ანგარიშს არ ჩანს; სეზონის ბოლო ეპიზოდი პროგრესს 100%-ს აყენებს — `EpisodeTest::test_a_mark_is_mine_and_invisible_to_another_account` და `::test_finishing_a_season_puts_it_at_a_hundred_percent`
- **Estimate:** L
- **დამოკიდებულება:** none

### [FEAT-10] „მალე" — კალენდარი
- **სტატუსი:** ✅ შესრულებულია (2026-09-19). `series.next_air_at`/`next_season`/`next_episode` (+ იგივე `animes`-ზე) სინქრონზე ივსება TMDB-ის `next_episode_to_air`-იდან, `GET /api/upcoming?days=` (სერვისი **`Services\Calendar\Upcoming`**) და დეშბორდის ბლოკი `components/UpcomingCard.tsx` — ბარათებზე **მაღლა**, რადგან თარიღიანი ინფორმაცია დროში ფუჭდება.
  - ⚠️ **დამატებითი TMDB-გამოძახება არ სჭირდება**: `next_episode_to_air` `tvDetails()`-ის პასუხშივე მოდის, ე.ი. კალენდარი ჩვეულებრივი სინქრონის გვერდითი პროდუქტია.
  - ⚠️ **ველი ყოველთვის გადაიწერება და არა `?:`-ით ივსება.** დანარჩენი ველები „მხოლოდ ცარიელზე" იწერება (მომხმარებლის ტექსტს ვერ გადავაწერთ), ეს კი **მოძრავი ფაქტია**: შენარჩუნებული ძველი თარიღი კალენდარში სამუდამოდ წარსულში იდგებოდა. `null`-ზე წაშლაც აუცილებელია — დასრულებულ სერიალს შემდეგი ეპიზოდი აღარ აქვს.
  - ⚠️ **`tv_episodes.air_date` (FEAT-09) განზრახ **არ** გამოიყენება მეორე წყაროდ**: ის მხოლოდ ცხადი ჩამოტვირთვის შემდეგ არსებობს, ე.ი. კალენდარი სრულიად სხვა ქმედების მიხედვით ჩნდებოდა და ქრებოდა.
  - ⚠️ **წიგნი, ბორდგეიმი და ფილმი გარეთ არიან და ეს არ არის გამორჩენა** — მათ მხოლოდ `year` აქვთ და არა თარიღი; 1 იანვრით ჩასმა გამოგონილი ფაქტი იქნებოდა.
  - ⚠️ **ჩანიშვნა შიგნითაა** (`due_at`): ის დღეს მხოლოდ თავის სექციაში ჩანს, ე.ი. სხვა მოდულში მუშაობისას უხილავია. §16.5 ჩანიშვნებს **საჯარო პროფილზე** კრძალავს და არა ჩემსავე დეშბორდზე.
  - ⚠️ **„ვერაფერი ვნახე" (2026-09-19) ორი მიზეზი ჰქონდა და ორივე გასწორდა.** პირველი — `next_air_at` სვეტიც არ არსებობდა (მიგრაცია გაუშვებელი იყო, იხ. FEAT-09), ე.ი. `/api/upcoming` ვარდებოდა და ბარათი უხმოდ ქრებოდა. მეორე და ნამდვილი ბაგი — **`ItemSyncer`-ს ეს სამი სვეტი საერთოდ არ ეწერა**: ჩამატება და თითოეული `resync` `TvEnricher`-ზე გადის (სადაც `applyNextAir()` ეწერა), `/sync`-ის რიგი კი `ItemSyncer`-ზე, ე.ი. **მთელი ბიბლიოთეკის სინქრონიზაციის შემდეგაც კალენდარი ცარიელი რჩებოდა**. ⚠️ `ItemSyncer::applyNextAir()` განზრახ **არ იყენებს ფაილის `$set()`-ს** („ცარიელს ავსებს, შევსებულს არ ეხება") — ეს მოძრავი ფაქტია და `overwrite`-საც არ ეკითხება. ტესტი `UpcomingTest::test_the_bulk_sync_fills_and_clears_the_next_air_date` შემოწმებულია: გასწორების გარეშე წითლდება.
  - ⚠️ **ფანჯარა დღის დასაწყისიდან ითვლება და არა „ახლა"-დან** — დღეს საღამოს გასული ეპიზოდი დღესვე უნდა ჩანდეს.
  - ⚠️ **„S05E03" ინტერფეისში იგება**: სერვერი ორ რიცხვს აბრუნებს, თორემ ენაზე დამოკიდებული ტექსტი ორ ადგილას იარსებებდა.
  - ⚠️ **`Http::fake()` stub-ებს **ამატებს** და არ ცვლის** (`Factory::fake()` → `stubUrl()`): მეორედ გამოძახებული fake ჩუმად არაფერს ცვლის, პირველი დამთხვეული იგებს. ტესტში პასუხი ამიტომ ერთია და იცვლება ცვლადი — ეს ხაფანგი ზუსტად აქ დაიჭირა.
  - ℹ️ **ავტომატური `note_reminder` („ეთერში გასვლის დღეს") არ აშენებულა** — ტასკშივე „არჩევითად" იყო მონიშნული და მას per-user პარამეტრი და შეხსენების დისპეტჩერზე მიბმა სჭირდება; ეს ცალკე ამოცანაა.
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] დეშბორდზე მომდევნო 30 დღის სია თარიღით და მოდულის ფერით — `UpcomingTest` (6 ტესტი)
- **Estimate:** M
- **დამოკიდებულება:** FEAT-09

### [FEAT-11] კალათა ჩანაწერებზე
- **სტატუსი:** ✅ შესრულებულია (2026-09-20). `trashed_at` ათივე ჩანაწერიან ცხრილზე, რეესტრი **`App\Support\TrashDomain`**, ქცევა **`Concerns\HasTrash`** (global scope `trash`), `GET /api/trash` · `POST /api/trash/{domain}/{id}/restore` · `DELETE /api/trash/{domain}/{id}` · `DELETE /api/trash` (აკრეფილი `DELETE`), გვერდი **`/trash`** და ბრძანება **`trash:prune`** (`Schedule` → ყოველდღე 03:30).
  - ⚠️ **`delete()` განზრახ **არ** გადაფარულა და ეს მთავარი გადაწყვეტილებაა.** `SoftDeletes`-ის სტილის გადაფარვა ერთდროულად შეცვლიდა **ყველა** წაშლას პროექტში — `PurgeService`-ს, ანგარიშის წაშლას (BUG-21), ლექსიკონის „ჩანაწერებიც წაშალე"-ს და ყველა კასკადს. კალათაში გადატანა ცხადი გამოძახებაა (`moveToTrash()`) და მას მხოლოდ მოდულის `destroy()` იძახის.
  - ⚠️ **გამომდინარე წესი:** *ჩვეულებრივი წაშლა კალათაშია, აკრეფილი `DELETE`-ის უკან — ნამდვილი.* `/purge` და ლექსიკონის „ჩანაწერებიც წაშალე" ტიპიზებულ დადასტურებაზე დგანან.
  - ⚠️ **კალათა არაფერს ათავისუფლებს** — ფაილი, ჩანიშვნა, გალერეა, pivot და კვოტა ადგილზეა, ე.ი. აღდგენა ნამდვილად უფასოა; სწორედ ამიტომ არ არის ის `delete()`.
  - ⚠️ **`PurgeService` scope-ს ცხადად თიშავს** — გამოტოვება „წაშალე ყველაფერი"-ს ნაწილს ჩუმად დატოვებდა და `plan()`-ის „გათავისუფლდება N ბაიტი" ტყუილი იქნებოდა.
  - ⚠️ **Laravel-ის `model:prune` არ გამოიყენება**: `Prunable` `SoftDeletes`-ს ელოდება და გლობალურ სკოუპებს **არ** თიშავს — ე.ი. ჩვენი `trash` scope სწორედ იმ რიგებს დამალავდა, რომელთა წაშლაც მას ევალება, და ბრძანება ყოველ ღამე წარმატებით გაეშვებოდა და ყოველთვის ნულს დაითვლიდა.
  - ⚠️ **ჟურნალი ხელით იწერება `moveToTrash()`-ში** — `deleted` მოვლენა არ ისვრება, ე.ი. „ვინ წაშალა" 30 დღე უპასუხოდ დარჩებოდა. აღდგენას საკუთარი მოქმედება აქვს (`AuditLog::ACTION_RESTORE`).
  - ⚠️ **ათი ტესტი შეიცვალა და ყველა მათგანი სწორად შეცვლილი ქცევა იყო**: ისინი `DELETE /api/<module>/{id}`-ის შემდეგ ფაილისა და კვოტის გათავისუფლებას ამოწმებდნენ. ახლა სრულ გზას გადიან (წაშლა → კალათა → საბოლოო წაშლა), ე.ი. დაფარვა გაიზარდა.
  - ⚠️ **`RegistryConsistencyTest::test_every_record_module_has_a_trash`** ახალ მოდულს ჩუმად შეუქცევადი წაშლით დარჩენის საშუალებას არ აძლევს; `gallery` ერთადერთი, დოკუმენტირებული გამონაკლისია.
- **ტიპი:** feature
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-20). ბრძანება **`backups:auto`** (`--keep`, `--force`), ახალი წყარო `DatabaseBackup::SOURCE_SCHEDULE`, `Schedule::command('backups:auto')->dailyAt(config('mediary.backup.at'))`, პარამეტრები `config/mediary.php`-ში (`BACKUP_AUTO`, `BACKUP_AUTO_KEEP`, `BACKUP_AUTO_AT`), `/backups`-ზე მდგომარეობის ზოლი და `mediary:doctor`-ში „ბოლო ასლი N დღის წინ".
  - ⚠️ **გადამრთველი `.env`-შია და არა ღილაკი გვერდზე** — ეს ინსტალაციის პარამეტრია და არა მომხმარებლის: ორი სუპერ-ადმინის შემთხვევაში „ვისი გადამრთველია" კითხვას პასუხი არ აქვს, ცალკე გლობალური ცხრილის დაბადება კი ერთი ბულევისთვის ზედმეტი მექანიზმია. გვერდი მას **მხოლოდ აჩვენებს** — `UploadLimitsCard`-ისა და `/credentials`-ის `meta.installation`-ის იგივე წესი.
  - ⚠️ **ფონურ პროცესს არ იშვებს**: `POST /admin/backups` `BackgroundProcess`-ზე გადის, რადგან `artisan serve` ერთძაფიანია; აქ კი უკვე კონსოლში ვართ, ე.ი. პროცესის გაშვება მხოლოდ ერთი ზედმეტი ჩავარდნის წერტილი იქნებოდა (`ProcessEnv`-ის გაკვეთილი).
  - ⚠️ **მფლობელი პირველი სუპერ-ადმინია** — ასლს ვიღაცის კვოტა უნდა გადაიხადოს (§22-ის წესი), ხოლო `/backups` ისედაც `super_admin`-ია.
  - ⚠️ **`keep` მხოლოდ `schedule` წყაროს ჭრის** — ხელით აღებული და ატვირთული ასლი ადამიანის გადაწყვეტილებაა და ავტომატურად არასდროს იშლება; წაშლა მოდელით ხდება, ე.ი. `StoredFile` ფაილსაც შლის და კვოტასაც ათავისუფლებს.
  - ⚠️ **doctor-ის შემოწმება scheduler-ისა არ არის და სწორედ ამიტომ არსებობს**: „ტასკი დარეგისტრირებულია" და „ასლი გაკეთდა" ორი სხვადასხვა ფაქტია — გარე PowerShell-ტასკი ექვსიდან ოთხ დღეს უხმოდ არ ეშვებოდა, მიუხედავად იმისა, რომ არსებობდა. ერთადერთი მტკიცებულება რიგია ბაზაში.
  - ⚠️ **კალათის გასუფთავება 03:30-ზეა და ასლი 03:00-ზე** — ასლი ყოველთვის *წაშლამდე* უნდა გაკეთდეს.
- **ტიპი:** feature
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-20). `messages.record` (JSON), `Message::TYPE_RECORD`, სერვისი **`Services\Chat\SharedRecord`**, გაგზავნა იმავე `POST /api/chat/{conversation}`-ზე (`domain` + `record_id`), დამატება **`POST /api/chat/messages/{message}/save`**, ინტერფეისი `components/chat/SharedRecordCard.tsx` + `ShareRecordDialog.tsx` (ღილაკი ჩანაწერის გვერდზე).
  - ⚠️ **გაზიარება ხილვადობას არ ცვლის** — ეს ტასქის მთავარი წესია: საჯარო ჩანაწერი სრული `PublicDomain::card()`-ით ჩანს, პირადიდან კი **მხოლოდ სათაური და იდენტობა** მიდის. ე.ი. „ეს ნახე" პირად ჩანაწერზეც მუშაობს.
  - ⚠️ **ბარათი კითხვის მომენტში იგება და არ ინახება** — გაყინული ასლი მოგვიანებით დაპრივატებულ ჩანაწერს ძველ წერილში სამუდამოდ ღიად დატოვებდა. სათაური მაინც ინახება: ჩანაწერი შეიძლება წაიშალოს.
  - ⚠️ **ინახება იდენტობა და არა `id`** — `movies.id` ჩემი ბიბლიოთეკის ნომერია და მიმღებთან არაფერს ნიშნავს. სია `PublicDomain::MATCH`-ია (იგივე ცხრა დომენი) და **`note` მასში არასდროს ყოფილა** (§16.5).
  - ⚠️ **ცალკე სვეტია და არა `attachment_*`-ის ხელახლა გამოყენება**: დანართი ფაილია (გზა, ზომა, MIME, კვოტა), გაზიარებული ჩანაწერი კი მითითება — და საერთო სვეტი `Message::attachmentDeleted()`-ს გააფუჭებდა.
  - ⚠️ **იმავე endpoint-ზეა**: `file` უკვე არჩევს ტექსტსა და ატვირთვას შორის, `domain` მესამე ტოტია; ცალკე route დაბლოკვისა და საჯაროობის კარიბჭეს მესამედ გაიმეორებდა.
  - ⚠️ **მხოლოდ საკუთარი ჩანაწერი იზიარება** (სხვისი id — 404): თორემ უცხო ბიბლიოთეკის სათაური გაჟონავდა, ხოლო სწორედ სათაურია ის, რაც პირად ჩანაწერზეც მიდის.
  - ⚠️ **`Bubble`-ში `m.record` დანართზე **მაღლა** მოწმდება** — `record`-ტიპის წერილს `attachment` არ აქვს, ე.ი. ქვემოთა პირობა მას ჩუმად ცარიელ ბუშტად დახატავდა.
  - ⚠️ **გამდიდრება ბონუსია და არა პირობა** — TMDB შეიძლება არ იყოს; ჩანაწერი მაინც იქმნება იდენტობითა და სათაურით (`sync_status = 'partial'`), რომ სინქრონმა შეავსოს. `MediaDomain::enrich()` სწორედ ამისთვის დაემატა: სამ გამამდიდრებელს სამი სხვადასხვა მეთოდი ჰქვია.
  - ℹ️ **გაზიარების ღილაკი ჯერ მედია-ჩანაწერის გვერდზეა** (ფილმი · სერიალი · ანიმე — ერთი გვერდი, სამი დომენი); დანარჩენ მოდულებში ის ერთი ღილაკის დამატებაა, რადგან დიალოგი დომენით პარამეტრიზებულია.
- **ტიპი:** feature
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-20). ცხრილი `media_watches` (პოლიმორფული), ქცევა **`Concerns\HasWatchLog`**, `GET|POST /api/media/watches/{type}/{id}` + `DELETE /api/media-watches/{watch}`, ბლოკი `components/WatchLog.tsx` ჩანაწერის გვერდზე. არსებული `watched_at` მიგრაციამ პირველ რიგად გადმოიტანა.
  - ⚠️ **`watched_at` რჩება და „ბოლო ნახვას" ნიშნავს** — მისი წაშლა ყველა ფილტრს, სორტირებას, `MatchService`-ს, `PurgeService`-სა და სტატისტიკას გადაწერას მოითხოვდა; სამაგიეროდ მისი **მწერელი ერთია** (`syncWatchedAt()` → `max(media_watches.watched_at)`).
  - ⚠️ **პირველ რიგს სტატუსი ქმნის და არა ღილაკი** — „ნანახად მოვნიშნე" უკვე ნახვაა; ცალკე ჩაწერის მოთხოვნა ჟურნალს ყველასთვის ცარიელს დატოვებდა.
  - ⚠️ **ეს ჩაწერა `saved` მოვლენაზეა და არა `HasStatus::applyStatus()`-ში**: იქ ახალ ჩანაწერს **ჯერ id არ აქვს**, ე.ი. პოლიმორფული რიგი ვერსად მიება.
  - ⚠️ **`firstOrCreate` იმავე წამზე** — სტატუსის ორჯერ მინიჭება (ფორმის ხელახალი შენახვა, მასობრივი ცვლილება) სხვაგვარად „ორჯერ ვნახე"-ს გამოიგონებდა.
  - ⚠️ **ბოლო ნახვის წაშლა `watched_at`-ს წინა რიგზე აბრუნებს**, ცარიელ ჟურნალზე კი `null`-ად — ეს ზუსტად „აღარაა ნანახი"-ს ნიშნავს.
  - ⚠️ **სტატისტიკის თვეები ჟურნალიდან იკითხება** (`LibraryStats::byWatchLog()`) — **ეს არის FEAT-08-ზე დამოკიდებულების აზრი**: „წელს რამდენი ვნახე" გამეორებებს ვერ ითვლიდა. `/stats`-ც და დეშბორდის ჯამიც ერთ წყაროზე დგას.
  - ⚠️ **მომავალი თარიღი 422-ია** — ჟურნალი წარსულის ჩანაწერია.
  - ⚠️ **მხოლოდ მედია-დომენები** — ვიდეოს საკუთარი `watch_count` აქვს (დაკვრის მრიცხველი, სხვა ფაქტი), წიგნსა და თამაშს კი „როდის დავასრულე" სვეტადაც არ აქვთ.
- **ტიპი:** feature
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-20). `public/manifest.webmanifest`, `public/app-icon.svg`, `<link rel="manifest">` + ორი `theme-color` (`index.html`) და ღილაკი `/profile`-ზე (`components/InstallApp.tsx`).
  - ⚠️ **SW-ში ქეში არ დაემატა და ეს ტასქის არსია, გამორჩენა არა.** მარშრუტები `lazy()`-ია, ე.ი. დაქეშილი ძველი `index.html` ახალ დეპლოიზე აღარარსებულ ჩანქს მოითხოვდა — ზუსტად ის ჩავარდნა, რისთვისაც `ErrorBoundary` დაიწერა, ოღონდ **ყველა ტაბზე და გადატვირთვის შემდეგაც**. Chrome-ს დიდი ხანია `fetch`-ჰენდლერი „installable"-ისთვის აღარ სჭირდება; თუ ძველი ბრაუზერი მოითხოვს, სწორი პასუხი **network-only** გამტარია და არა ქეში.
  - ⚠️ **`theme-color` ორი მეტაა და არა manifest-ის ერთი მნიშვნელობა** — თორემ მუქ თემაზე ბრაუზერის ზოლი ნათელი ლაქა იქნებოდა. ორივე `index.css`-ის `--background`-ია.
  - ⚠️ **ხატულა ცალკე ფაილია** — `favicon.svg` ფილტრებით აგებული ილუსტრაციაა და მთავარი ეკრანის ზომაზე ლაქა ხდება. ლოგო **ცენტრალურ 60%-შია** (maskable-ის უსაფრთხო ზონა); გადამოწმებულია რიცხვით: კონტურის ყველა კუთხე ცენტრიდან 195px-ზეა, ზღვარი 204.8-ია.
  - ⚠️ **iOS manifest-ის ხატულას არ კითხულობს** — მას `apple-touch-icon` და **მხოლოდ PNG** სჭირდება; რეპოში რასტერიზატორი არ არის, ე.ი. iPhone-ზე ხატულა ეკრანის სურათია. standalone-რეჟიმი, სახელი და ფერი იქაც მუშაობს.
  - ⚠️ **ღილაკი მხოლოდ `beforeinstallprompt`-ზე ჩნდება** — ის არ ისვრება დაყენებულ აპზე, არასრულ manifest-ზე და Safari/Firefox-ზე საერთოდ; მუდმივი ღილაკი სამივე შემთხვევაში იტყუებოდა.
  - 🟡 **„Lighthouse Installable მწვანე" ამ სესიიდან არ გაშვებულა** — manifest, ხატულა, `display: standalone` და `start_url` ადგილზეა და `dist/`-შიც გადის, მაგრამ თვითონ აუდიტი Chrome-ს ითხოვს.
- **ტიპი:** feature
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-21). ორივე ნახევარი აშენდა.
  **(1) აღდგენის ბმული:** `App\Support\ResetLink` + `POST /admin/users/{id}/reset-link` (`admin_access:users`, SEC-02-ის `outranks()` კარით) და საჯარო წყვილი `GET|POST /auth/reset/{token}` (`throttle:login`-ის უკან). ტოკენი 48 სიმბოლოა, ბაზაში მხოლოდ `sha256` წერია (`bcrypt` გამოსადეგი არ იყო — ძებნა გასაღებით ხდება, მარილიანი ჰეში მთელ სკანს მოითხოვდა), ცხრილი Laravel-ის `password_reset_tokens`-ია (`email`-ზე დაპირველადებული = ერთ ანგარიშზე ერთი მოქმედი ბმული) და `token`-ს დაემატა `unique` ინდექსი. ვადა 24 სთ; ვადაგასული/გამოყენებული — **410 `reset_link_expired`**. პაროლის ხელით შეცვლაც კლავს ბმულს. UI: `/reset/:token` (`Protected`-ისა და `GuestOnly`-ის გარეთ) + ბლოკი `/users/{id}`-ზე.
  **(2) TOTP:** `App\Support\Totp` — RFC 6238 **ხელით**, ბიბლიოთეკის გარეშე (~40 ხაზი, სტანდარტის საკუთარი ტესტ-ვექტორებით შემოწმებული); `users.two_factor_secret`/`_recovery_codes` **დაშიფრული** (`APP_KEY`), `two_factor_confirmed_at` ცალკე ფაქტია — ჩართვა ორნაბიჯიანია და დადასტურებამდე შესვლა არ იცვლება. `/auth/2fa[/confirm|/recovery-codes]` + `DELETE` — სამივე **ანგარიშის პაროლს** ითხოვს. შესვლა: **409 `two_factor_required`** → კოდი იმავე მოთხოვნაში (სერვერზე „ნახევრად შესული" მდგომარეობა განზრახ არ არსებობს). აღდგენის კოდი ერთხელ მუშაობს. QR — `qrcode.react` (0 დამოკიდებულება, lazy ჩანქი 16 kB).
  ⚠️ **გვერდითი ორი გამოსწორება:** `login()` ახლა ცხადად `Auth::guard('web')`-ს იყენებს (`RequestGuard`-ს `validate()`/`getLastAttempted()` არ აქვს), და `copyText()` `lib/clipboard.ts`-ში გამოვიდა (სამი გამომძახებელი).
  ტესტი: `AccountRecoveryTest` (12).
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] ვადაგასული/გამოყენებული ბმული 410-ია; ტოკენი ბაზაში მხოლოდ ჰეშად ინახება
  - [x] 2FA-იან ანგარიშზე პაროლი მარტო არ შედის; აღდგენის კოდი ერთხელ მუშაობს
- **Estimate:** L
- **დამოკიდებულება:** SEC-15

### [FEAT-17] დუბლის გაფრთხილება ვიდეოს/სიმღერის დამატებაზე
- **სტატუსი:** ✅ შესრულებულია (2026-09-21). `App\Support\DuplicateLink` + `existing` ველი `POST /videos/metadata`-სა და `/songs/metadata`-ს პასუხში; UI — `components/DuplicateLinkNotice.tsx` (ერთი კომპონენტი ორივე ფორმაზე) „გახსნა" ღილაკით, რომელიც ჩანაწერს **id-ით** მოაქვს (დუბლი შეიძლება მიმდინარე ფილტრს მიღმა იყოს). იდენტობა `platform`+`external_id`-ია (`PublicDomain::MATCH`-ის წყვილი), ე.ი. `youtu.be/…` და `watch?v=…` ერთი და იმავე ვიდეოდ იკითხება; პირდაპირ ფაილზე (`external_id = null`) შედარება მისამართითაა — **`null`-ზე შედარება აკრძალულია**, თორემ ყველა პირდაპირი ბმული ერთმანეთის დუბლი გახდებოდა. `exclude` რედაქტირებისთვისაა (ჩანაწერი საკუთარი თავის დუბლი არაა), `owner`/`trash` scope-ები ძალაშია. **მკაცრი `unique` არ დამატებულა** — ტასქის ცხადი გადაწყვეტილება. ტესტი: `DuplicateLinkTest` (9). ⚠️ ბუკმარკზე **განზრახ არ გავრცელდა** (ტასქის მოცულობა ვიდეო/სიმღერაა); `DuplicateLink` დომენისგან დამოუკიდებელია, ე.ი. საჭიროების შემთხვევაში ერთი ხაზია.
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] არსებულ `external_id`-ზე `metadata` `existing`-ს აბრუნებს და ფორმა ბმულს აჩვენებს; შენახვა მაინც შესაძლებელია
- **Estimate:** S
- **დამოკიდებულება:** none

### [FEAT-18] პირადი ტეგები მედია-დომენებზე
- **სტატუსი:** ✅ შესრულებულია (2026-09-21). `tags` JSON სამივე ცხრილზე, `Concerns\HasTags` (ერთი ქცევა ცხრა მოდულზე — **ხუთი ძველი ასლიც** მასზე გადავიდა), ფილტრი (`?tag=`, მძიმით გამოყოფილი, **AND**), `GlobalSearch`-ის საერთო `$media` აღწერაში `json` (ქართული ესკეიპი `jsonLike()`-ს უკვე ჰქონდა), `PurgeService::TARGET_MODES`-ში `tag` (+ frontend-ის `PURGE_TARGET_MODES`), `FieldCatalog::MEDIA_FIELDS`-ში ველი, `ExportDomain`-ში სვეტი, რესურსებში `tags`. შემოთავაზების სია — **`GET /api/media/tags`** (`GET /games/franchises`-ის ფორმა: ლექსიკონის ცხრილი არ არსებობს, სია სვეტიდან გროვდება; `all=1`-ით სიის ჩამოტვირთვა უარყოფილია). UI: `MovieFormPage`-ზე ცალკე `TagSelect` ბლოკი (ჟანრისგან **განცალკევებით**) და `LibraryPage`-ის ფილტრში ცალკე ჯგუფი. ტესტი: `MediaTagsTest` (8).
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] ფილმზე ტეგი ინახება, ფილტრი AND-ით მუშაობს, ძებნა ტეგშიც პოულობს (ქართულ ესკეიპზეც)
- **Estimate:** M
- **დამოკიდებულება:** BUG-16

### [FEAT-19] შეტყობინებების ცენტრი
- **სტატუსი:** ✅ შესრულებულია (2026-09-21). Laravel-ის `notifications` ცხრილი (database არხი), `App\Support\NotificationType` (ხუთი სახე + მარშრუტი), `Services\Notify\Notifier` (ერთადერთი ჩაწერის წერტილი, **არასდროს აგდებს გამონაკლისს**), `App\Notifications\AppNotification` (ერთი კლასი — `databaseType()` სახეს წერს კლასის სახელის ნაცვლად) და `GET|PATCH|DELETE /api/notifications*`. ოთხივე მოვლენა ჩართულია: მოთხოვნის დამტკიცება/უარყოფა, კვოტის **ზღვრის გადალახვა** (მდგომარეობა არა — თორემ ყოველი ატვირთვა გაიმეორებდა), ჩავარდნილი ასლი (**ყველა** სუპერ-ადმინს) და დასრულებული პარტია (`finally`, არა `then` — `allowFailures()`-ის გამო). UI: `components/NotificationBell.tsx` ჰედერში, ბეჯი 30 წამზე მხოლოდ **რიცხვს** ეკითხება. ⚠️ `'user' => User::class` **morph რუკას დაემატა** — უამისოდ ყოველი შეტყობინება 500 იყო. ⚠️ **ტელეგრამის არხი განზრახ არ ჩართულა** (ტასქშიც „არჩევითი“). ტესტი: `NotificationTest` (10).
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] მოთხოვნის დამტკიცება მომხმარებელს შეტყობინებას უტოვებს; წაკითხვა `read_at`-ს წერს; ბეჯი წაუკითხავს ითვლის
- **Estimate:** M
- **დამოკიდებულება:** none

### [FEAT-20] „რა ვნახო დღეს" — შემთხვევითი არჩევანი
- **სტატუსი:** ✅ შესრულებულია (2026-09-21). `?pick=random` სამივე მედია-დომენის `index()`-ზე — **ფილტრების შემდეგ და დალაგებამდე**; ნაგულისხმევად `statusRole(todo)` (ცხადად არჩეული `?status=` მასზე მაღლა დგას), `reorder()->inRandomOrder()->first()`, ცარიელზე **`data: null`** და არა 404. UI — კამათლის ღილაკი ბიბლიოთეკის ჰედერში (ფილტრის ჩამრთველის გვერდით) და `components/RandomPickDialog.tsx`: ბარათი, „სხვა" და „დავიწყოთ" (სტატუსი `doing` **როლით**; ასეთის არარსებობისას ღილაკი არ იხატება). ⚠️ არჩევანი სერვერზე ხდება — ბადეში მხოლოდ პრეფიქსია ჩატვირთული, ე.ი. ბრაუზერში არჩეული „შემთხვევითი" პირველ სამოცში მოხვდებოდა. ტესტი: `RandomPickTest` (6).
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] ღილაკი ფილტრის ფარგლებში შემთხვევით ჩანაწერს აჩვენებს; ცარიელ ფილტრზე `EmptyState`
- **Estimate:** S
- **დამოკიდებულება:** none

### [FEAT-21] წლიური მიზნები
- **სტატუსი:** ✅ შესრულებულია (2026-09-21). მიზანი `users.settings.goals`-შია (`{module: {year: target}}`, მიგრაციის გარეშე — **წელი გასაღებია**, ე.ი. შარშანდელი ისტორიად რჩება), პროგრესი კი `GET /api/stats/summary`-ის ახალი `done_by_module`-იდან (**ცალკე query არ დამატებულა** — თვეების ციკლში იკრიბება). UI: `components/YearGoals.tsx` დეშბორდზე, სტატისტიკის ბლოკის ქვემოთ; ზოლის ფერი `modules.color`-იდან.
...
- **სტატუსი:** ✅ შესრულებულია (2026-09-20). მეთორმეტე მოდული `place`. ოთხი ცხრილი: `places` · `place_categories` (per-user ლექსიკონი) · `place_files` (`image`/`doc`) · `place_field_values`. წყარო — **OSM Nominatim** (`Services\Places\NominatimClient`), გასაღების გარეშე. ⚠️ **`User-Agent` სავალდებულოა და წამში ერთი მოთხოვნაა ნებადართული** — ქეში + `pace()`; ძებნა ცხადი ღილაკია და არასდროს გვერდითი ეფექტი. ⚠️ **კოორდინატი `decimal(10,7)`-ია და არა `float`**; **ნული ნამდვილი კოორდინატია** (`?:`-ით დაწერილი შევსება მას ჩუმად გადააგდებდა). ⚠️ **`visited_at`-ს მხოლოდ `Place::applyStatus()` წერს**; უკან დაბრუნებაზე იშლება. ⚠️ **ადგილი `GalleryParent`-შია** — ვებძებნა და გალერეის ჭრილები უფასოდ ემატება; ჩემი ატვირთული ფოტო — `place_files`, ვებიდან მოტანილი — `gallery_images`. ⚠️ **რუკა არ დაემატა** (ტასქის საკუთარი სიტყვა: „მოგვიანებით“) — Leaflet ~40 kB-ია; კოორდინატი ინახება და OSM-ის ბმულად ჩანს. UI: `/places` (სია, ფილტრი, ფორმა Nominatim-ის ძებნით), `components/PlaceDetail.tsx`, ლექსიკონი `/dictionaries/place-categories`. ტესტი: `PlaceModuleTest` (18); `RegistryConsistencyTest` მწვანეა.
- **ტიპი:** feature
...
- **Acceptance criteria:**
  - [x] ადგილის ძებნა Nominatim-ით და ხელით; ფოტოები გალერეაში ჩანს; მატჩინგი `osm_id`-ით ითვლის
- **Estimate:** L
- **დამოკიდებულება:** none
