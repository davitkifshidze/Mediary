...
final class AlbumLock
{
    ...
    /**
     * **სესია საერთოდ არსებობს? (Tasks BUG-02)**
     *
     * ⚠️ ეს კითხვა `unlock()`-ის გამომძახებელს სჭირდება და არა თვითონ ლოკს:
     * სესიის გარეშე `unlock()` უხმაუროდ `return`-ს აკეთებდა, კონტროლერი კი
     * მაინც „გავხსენი"-ს აბრუნებდა — ე.ი. სწორი პაროლის შეყვანა 200-ს
     * იძლეოდა და ალბომი ჩაკეტილი რჩებოდა, ახსნის გარეშე. ახლა ის 409-ია.
     *
     * ⚠️ პასუხი **ერთ ადგილას** ცოცხლობს, რადგან „როდის აქვს `/api`-ს სესია"
     * ზუსტად ის ფაქტია, რომელსაც ქვემოთ `session()` კითხულობს; მეორე ასლი
     * (`$request->hasSession()` კონტროლერში) იმავე დღეს გაიშლებოდა.
     */
