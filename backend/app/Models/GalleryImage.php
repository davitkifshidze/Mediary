...
class GalleryImage extends Model
{
    ...
    /**
     * **ეს ფოტო ჩაკეტილ ალბომშია? (2026-09-20)**
     *
     * შენი სიტყვები: „როდესაც ჩაკეტილ კატეგორიაში იქნება, ყველა ფოტოში
     * ჩანდეს დაბლარულად და თუ პაროლს არ შეიყვან, არ გამოჩნდება".
     *
     * ⚠️ **scope-ს ეს არაფერს აკლებს** — ის ისევ **ყველგან** მალავს
     * ჩაკეტილ რიგს. ეს კითხვა იმ ორ სიას სჭირდება, სადაც რიგი განზრახ
     * შემოდის (`GalleryController::photos()` და `show()`): იქ ფოტო
     * **ჩანს, მაგრამ შიშველი** — `GalleryImageResource` სწორედ ამ პასუხზე
     * ტოვებს `path`-ს, `remote_path`-სა და `source_url`-ს პასუხის გარეთ.
     *
     * ⚠️ **პასუხი `AlbumLock`-ისაა და არა აქაური `where`-ისა**: იქ სია
     * მემოიზებულია, ე.ი. 24-ფოტოიანი გვერდი ერთ `select`-ს აკეთებს და
     * არა ოცდაოთხს.
     */
    public function isAlbumLocked(): bool
    {
        return $this->album_id !== null
            && in_array((int) $this->album_id, AlbumLock::hiddenIds(), true);
    }
