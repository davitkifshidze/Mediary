...
abstract class TvEnricher
{
     ...
     * და `enrichSeries()` თითო ჩანაწერზე **ორ** Gemini-გამოძახებას ხარჯავდნენ
    ...
    public function draftFromId(int $id): array
    {
        ...
        return [
            ...
            'title_ka' => Lang::georgian($dka['name'] ?? null),
            'year' => $this->year($d),
    ...
    public function enrich(Model $series): bool
    {
        ...
        $titleKa = $curTitleKa ?: Lang::georgian($dka['name'] ?? null);
        $descKa = $curDescKa;
        $descKaSrc = $curDescKaSrc;
        if (! $curDescKa && ($ka = Lang::georgian($dka['overview'] ?? null))) {
            $descKa = $ka;
            // ⚠️ იხ. `MovieEnricher` — `'translated'` აპს არსად ეცნობოდა
            $descKaSrc = 'tmdb';
        }
